...
def bundle_to_trap_samples(obj: dict) -> tuple[torch.Tensor, float | None, dict, str]:
    """
    Returns:
    ...
    if "samples_X_trap" in obj:
        ...
        if omega is None:
            raise KeyError(
                "Found samples_X_bohr but no 'omega' in bundle; cannot convert to trap units."
            )
        ...
        if units == "trap":
            ...
            if omega is None:
                raise KeyError(
                    "Found 'X' without units==trap and no 'omega'—cannot convert to trap units."
                )
            y = X * math.sqrt(float(omega))
            used = "X (bohr→trap)"
    else:
        raise KeyError(
            f"No samples found. Available keys: {list(obj.keys())}. "
...
def _omega_dirname(omega: float | str) -> str:
    w = float(omega)
    return f"omega_{w:0.5f}"


def resolve_latest_pt(base: Path, N: int, omega: float | str) -> Path:
    """
...
def summarize_single_shell(
    X: torch.Tensor, mask_1shell: torch.Tensor, cfg: ShellAnalysisConfig
) -> dict:
    """
    ...
    if M == 0:
        return dict(
            ...
            M=0,
            K=K,
            phi=(float("nan"), float("nan")),
            lind=(float("nan"), float("nan")),
        )
...
def summarize_modal_shell_structure(
    X: torch.Tensor, shell_id: torch.Tensor, S: int, mask_S: torch.Tensor, cfg: ShellAnalysisConfig
) -> dict:
    """
    ...
    for s in range(S):
        ...
        if n_s < 3:
            ring_stats[s] = dict(
                n=n_s, phi=(float("nan"), float("nan")), lind=(float("nan"), float("nan"))
            )
            continue
...
def analyze_case(
    N: int,
    omega: float | str,
    cfg: ShellAnalysisConfig | None = None,
    ...
    if verbose:
        ...
        for S, out in modal_by_S.items():
            ...
            print(
                f"S={S} frames: {frames_S} | modal occ {occ_modal} occurs {modal_count} times "
            ...
            for s in range(S):
                ...
                if n_s < 3:
                    ...
                    lm, lse = st["lind"]
                    print(
                        f"  shell{s} (n={n_s}): |Φ_{n_s}|={pm:.3f}±{pse:.3f}, Lind={lm:.3f}±{lse:.3f}"
                    )
...
def run_many(
    ...
    if pairwise:
        for N, w in zip(Ns, omegas, strict=False):
