...
\S\ref{sec:colloc-results}; they expose the memory bottleneck at this size. The
...
expressive is beaten by the simpler one, consistent with the memory-forced capacity
cut degrading gradient quality more than its extra flexibility repays. Polishing the
...
the sub-percent accuracy reached at $N{=}6,12$ is consistent with the memory-forced
capacity reduction and the gradient noise it brings. A more memory-frugal backflow is
the direct next intervention; the present experiments do not isolate the collocation
objective as the source of the remaining gap.
