...
\chapter{Architecture, optimisation, and sampling geometry}
\label{ch:kernel}
...
indistinguishable in energy. At the crystal the two sets of runs do not overlap: three
CTNN runs give $(1.95,\,1.63,\,1.96)\times10^{-5}$ against three DeepSet runs at
$(3.40,\,3.23,\,3.41)\times10^{-5}$. Intermediate confinements
...
no longer does: at matched protocol and production capacity it is lower for the
message-passing ansatz at every confinement, by factors of two to seven
(Table~\ref{tab:varEL}), with non-overlapping seeds at the crystal. \emph{Controlled
