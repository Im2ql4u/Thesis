...
The previous chapter ended with four facts about this problem: the objective contains a
second-order operator, the interaction is singular and the state carries nodes, the
sampling measure is part of the method, and correlation is relational. This chapter builds
the state those facts allow.

Very little of what follows is a free choice. Antisymmetry is not optional, so there is a
determinant. The Coulomb divergence can be cancelled analytically, so it is---which means
the network never has to learn the one thing it would learn badly. Second derivatives enter
the loss, so every feature fed to the network is bounded and smooth, and every coordinate is
scaled so that the derivatives it produces are $\mathcal O(1)$ across three decades of
confinement. What is genuinely open is the form of the learned part, and there we build
both candidates and let the results decide. Unless stated otherwise we work in atomic
units.
...
singular. Solving the SR step without forming $S$ is what makes it affordable at
...
not the number of parameters, sets the true rank of the
...
weights~\cite{Vehtari2024}. The second deserves a word, because it is not merely a second
opinion on the first. Importance weights are heavy-tailed, and $\hat k$ estimates the tail
index of the generalised Pareto distribution fitted to the largest of them. The threshold
that matters is a statement about moments: for $\hat k<1/2$ the weights have finite
variance and the reweighted estimator obeys a central limit theorem; between $1/2$ and $1$
the variance is infinite but the mean still exists, and convergence is slow enough that
$\hat k\approx0.7$ is the practical limit of trustworthiness; above $1$ the mean itself is
undefined and no sample size repairs it. An ESS can look survivable while $\hat k$ says the
