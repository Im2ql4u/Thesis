...
def sr_step(all_params, psi_log_fn, x, E_L, damping=1e-3):
    ...
    for b in range(B):
        ...
        lp.backward()
        row = torch.cat([p.grad.flatten() if p.grad is not None
...
def train_vmc_sr(f, bf, C, p, *, n_epochs=50, lr=0.01, n_walkers=256,
                 damping=1e-3, step_sigma=0.15, clip_delta=1.0,
                 E_clip_width=5.0, label=""):
...
if __name__ == "__main__":
    ...
    print(f"CTNN is FRESH, bf_scale=0.05 (will grow via SR)")
    ...
    f_net, bf_net = train_vmc_sr(
        f_net, bf_net, C, p,
        n_epochs=150, lr=0.003, n_walkers=512,
        damping=1e-3, step_sigma=0.15, clip_delta=0.3,
        E_clip_width=5.0,
    ...
    print(f"  VMC+SR 150ep (CTNN only)         E={r1['E_mean']:.6f} +/- {r1['E_stderr']:.6f}  err={err1:.2f}%")
    print(f"  best collocation (bf_0.7 joint)  E=11.823300 +/- 0.002980  err=0.33%")
    print(f"  PINN-only collocation            E=~11.834   +/- ~0.003    err=0.42%")
    print("Done.")
