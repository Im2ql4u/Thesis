...
\begin{tabular}{@{}p{0.155\textwidth} p{0.185\textwidth} p{0.185\textwidth} p{0.115\textwidth} p{0.235\textwidth}@{}}
\toprule
Programme & Correlator & Backflow & Total & Supports \\
\midrule
Production (SR--VMC), Ch.~\ref{ch:results} &
  separable, $54$k &
  conventional $51$k, or message-passing $182$k &
  $105$k or $236$k &
  the energies of \S\ref{sec:energy-overview} and every structural claim \\[2pt]
Architecture study, \S\ref{sec:kernel-q1} &
  separable $47.7$k (matched: $89.4$k) \emph{vs.}\ message-passing $79.8$k &
  conventional $51$k \emph{vs.}\ message-passing $182$k (\emph{not} matched) &
  correlator ladder $20$--$164$k &
  the architecture claims and $\Var(E_L)$ \\[2pt]
Collocation, \S\ref{sec:kernel-mcmcfree} &
  message-passing, $25$k &
  message-passing, $+24$k (or none) &
  $25$--$49$k &
  the training-paradigm claims \\
\bottomrule
\end{tabular}
\end{center}

\noindent Widths and the remaining settings are in Table~\ref{tab:hyperparams} for the first
two and \S\ref{subsec:colloc-families} for the third. Two consequences are worth stating
plainly, because they are easy to lose. The physics of Chapter~\ref{ch:results} was produced
with the \emph{separable} correlator; the message-passing correlator appears only in the
architecture study and in the collocation programme, so no energy in Chapter~\ref{ch:results}
is evidence for or against it. And the two production ans\"atze share that correlator and
differ only in their backflow, which is why they are named \emph{conv.\ BF} and
\emph{MP BF} rather than by correlator type.
...
\medskip
\noindent\textbf{Naming.} The two learnable objects are named by function here, not by implementation class. The
\emph{correlator} is the Jastrow factor $W_\theta$, in a \emph{separable} form
(per-particle and pairwise embeddings pooled independently) or a \emph{message-passing} one;
the \emph{backflow} is the displacement field, in a \emph{conventional} form (one round of
pairwise messages) or a \emph{message-passing} one (persistent, transported edge state). The
corresponding code classes are specified in Chapter~\ref{ch:trial_wavefunction}.
