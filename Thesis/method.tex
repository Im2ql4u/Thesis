...
(i) $\varepsilon_{\rm bf}\in[10^{-6},10^{-4}]$ (trap units) for the backflow soft norm of
\eqref{eq:bf-message} --- distinct from, and far smaller than, the pair-feature softening
$\varepsilon\in[0.15,0.30]$ of \eqref{eq:safe-feats}, which sets a physical short-range
scale, not merely a removed kink;
...
larger---and where Chapter~\ref{ch:kernel} departs from matched capacity it does so in the
\emph{separable} network's favour, scaling the DeepSet from $20$k to $164$k parameters
against a fixed $80$k CTNN without closing the gap. All settings below are those of the accompanying code
...
\caption[Architecture and training hyperparameters]{Representative architecture and training hyperparameters. Widths are hidden
dimensions; ``rounds'' are message-passing steps ($n_\downarrow{+}n_\uparrow$ for the
V-cycle). The Params column gives the \emph{production} counts, which are the widths
carrying the energies of \S\ref{sec:energy-overview} and the $\Var(E_L)$ comparison of
Table~\ref{tab:varEL}; they are not matched between the correlator variants, and
\S\ref{sec:kernel-q1a} therefore also reports the capacity-matched $89.4$k DeepSet and the
full $20$k--$164$k ladder. The comparison-tier column gives the matched settings used for
the depth analysis.}
...
\hline
Correlator & CTNN (V-cycle)    & node/edge $32$, bottleneck $16$, $2{+}2$ rounds & width $128$, $2$ layers & $79.8$k \\
           & DeepSet           & pair-MLP $64\times4$, out $32$                  & width $128$, $2$ layers & $47.7$k \\
Backflow   & CTNN (copresheaf) & node/edge $64$, $1$ round                       & width $128$, $3$ layers & $83$k \\
           & Conventional      & msg $64$, node-MLP $64\times3$                  & width $128$, $3$ layers & $83$k \\
...
problem~\cite{ChenHeyl2024-minSR,Rende2024-KernelSR}. Under $|\Psi|^2$ sampling the Adam and SR \emph{fixed points} coincide; they differ
...
chain, so the choice of \emph{sampling measure} must be stated explicitly: the results
treat it as a variable in its own right. Two paradigms appear in this
...
\S\ref{sec:colloc-reliability}). Three controls manage the tails: weight tempering ($w\!\to\!w^{\alpha}$,
...
On each detected ring (inner or outer) we centre the mean angle to remove global rotations and define 
...
molecule is internally: it measures \emph{pinning} to the lab frame, not order. In the
\emph{co-rotating} frame each configuration is first registered by removing the phase of its
outermost shell, $\Phi_{n_s}\mapsto\Phi_{n_s}e^{-i\arg\Phi_{n_S}}$, before averaging:
\begin{equation}
  \tilde C_{\rm global} \;=\; \Big|\;\Big\langle \tfrac{1}{S'}\textstyle\sum_{s:\,n_s\ge3}
  \Phi_{n_s}\,e^{-i\arg\Phi_{n_S}}\Big\rangle_{\rm frames}\Big| .
  \label{eq:Ctilde}
\end{equation}
...
the shell of occupancy $n$ after the same registration, so that global rotation does not
inflate the measured spacing fluctuations.
