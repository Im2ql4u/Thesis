...
class KFACPreconditioner:
    ...
    def __init__(self, module: nn.Module, damping: float = 1e-3,
                 ema: float = 0.95, update_freq: int = 1):
...
def train_kfac(
    ...
    _orig_slogdet = None
    if use_damped_sd:
        import functions.Slater_Determinant as sd_mod
    ...
    for epoch_global in range(total_epochs):
        ...
        for i in range(0, n_collocation, micro_batch):
            ...
            # --- smoothness penalty ---
            if smooth_lambda > 0:
                pen = compute_bf_laplacian_penalty(bf_net, x_mb, spin, n_samples=min(32, x_mb.shape[0]))
...
def run_experiment(name, train_fn, C_occ, params, force=False):
    ...
    if not force:
        ...
        if loaded is not None:
            f_net, bf_net = loaded
            print(f"  Skipping training — using saved model")
...
def train_kfac_smooth(C_occ, params):
    ...
    f_net, bf_net = train_kfac(
        f_net, bf_net, C_occ, params,
        optimizer=optimizer, n_epochs=300,
        kfac_precond=kfac, smooth_lambda=1e-3,
