...
\emph{large}, and with these sample sizes the two come apart. Under the null $|\Phi_n|$ is
Rayleigh distributed with mean $0.886/\sqrt n$ and standard deviation $0.463/\sqrt n$, so
the standard error on the mean over $K$ frames is $0.463/\sqrt{nK}$. At $N{=}6$,
$\omega=0.1$, with $K=1.19\times10^6$ frames, that is $1.9\times10^{-4}$, and the $2\%$
excess is $46$ standard errors---resolved beyond any doubt, and still only $2\%$. Statistical
detectability is therefore not what separates the regimes here; magnitude is. On that
criterion the ordering is flat through $\omega=0.1$, weakly present at $\omega=0.01$ and
only at $N{=}6$, and unambiguous at $10^{-3}$.
...
angularly indistinguishable from randomly placed electrons; see the text for why
resolvability and magnitude come apart at these sample sizes. Single-particle inner shells are omitted from the
