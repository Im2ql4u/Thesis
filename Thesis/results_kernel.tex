...
\paragraph{The road.} We begin with what message passing changes in the tangent
geometry, in the amplitude and in the nodes; then ask what that geometry implies for the
optimiser and for the measure that builds it; then, taking the measure as something we
may choose, remove the Markov chain from training altogether and find where the choice
stops working. One caveat runs underneath all three and is itself a result: quantities
living on a single component---backflow rank, displacement magnitude---are gauge-like,
fixed by the training route rather than by the state. What the conclusions rest on
instead is set out in \S\ref{sec:synthesis}, once there are conclusions to rest.
...
This needs one reconciliation, because \S\ref{sec:colloc-reliability} below reports usable
collocation energies at the same $(N,\omega)$ where the estimator is called degenerate here
