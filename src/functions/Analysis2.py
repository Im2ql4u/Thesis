...
def compute_local_energy_batch(
    ...
    *,
    ...
    try:
        from training_sr import _local_energy_hvp as _leh, _local_energy_fd as _lef, _local_energy_exact as _lee
        if mode == "hvp":
    ...
    *,
    ...
    with torch.set_grad_enabled(track_grad):
        ...
        if getattr(f_net, "use_pair_attn", False):
            attn_rc = torch.as_tensor(getattr(f_net, "attn_rc", 1.0), dtype=x.dtype, device=x.device).clamp_min(1e-4)
            attn_p  = torch.as_tensor(getattr(f_net, "attn_p",  2.0), dtype=x.dtype, device=x.device).clamp_min(1.0)
    ...
    return dict(
        out=out, out_wo_cusp=out_base, cusp=cusp_sum,
...
def analyze_rho_weights(f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
    """
...
@torch.no_grad()
def feature_svd(f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
    """
...
@torch.no_grad()
def cusp_vs_residual_means(f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, float]:
    taps = forward_taps(f_net, X, spin)
...
@torch.no_grad()
def pca_on_rho_in(f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
...
def compute_physical_summaries(
    f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None, r0: float = 0.25, nbins: int = 24
) -> Dict[str, torch.Tensor]:
    """
...
def linear_probe_pcs(
    f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None, top_k: int = 3
) -> Dict[str, torch.Tensor]:
    """
...
@torch.no_grad()
def branch_ablation_drop(f_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, float]:
    """
...
def run_compact_analysis(
    ...
    *,
    ...
    spin: Optional[torch.Tensor] = None,
    backflow_net: Optional[nn.Module] = None,
    B_smallN: int = 16384,
    B_bigN: int = 8096,
    ...
    def _exact_local_energy(
        ...
        *, chunk_B: int = 96,   # tune: 64–192 for float64 on 24GB; 256–512 on float32
        ...
        for xb in x.split(chunk_B, dim=0):
            ...
            g = torch.autograd.grad(logpsi.sum(), xb, create_graph=True, retain_graph=True)[0]  # (b,N,d)
            ...
            for j in range(D):
                ...
                hj = torch.autograd.grad(sj, xb, retain_graph=True, create_graph=False, allow_unused=False)[0]
    ...
    # ====================== BF-ONLY HELPERS (NEW) ======================
    @torch.no_grad()
    def _bf_forward_taps(backflow_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None,
                         *, track_grad: bool = False) -> Dict[str, torch.Tensor]:
        with torch.set_grad_enabled(track_grad):
            B, N, d = X.shape
            ...
            if getattr(backflow_net, "use_spin", False) and (spin is not None):
                ...
                weight = same if getattr(backflow_net, "same_spin_only", False) else torch.ones_like(same)
            else:
            ...
            if   agg == "sum":  m_i = m_ij.sum(dim=2)
            elif agg == "mean": m_i = m_ij.mean(dim=2)
            elif agg == "max":  m_i = m_ij.max(dim=2).values
            else:               m_i = m_ij.sum(dim=2)
        ...
        return dict(msg_in=msg_in, m_ij=m_ij, m_i=m_i, upd=upd, dx_pre=dx_pre, dx=dx, weights=weight)

    def _cov_eigs(M: torch.Tensor) -> torch.Tensor:
        Mc = M - M.mean(dim=0, keepdim=True)
        B = Mc.shape[0]; F = Mc.shape[1]
    ...
    @torch.no_grad()
    def _bf_effective_ranks(backflow_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, float]:
    ...
    @torch.no_grad()
    def _bf_pca_on_disp(backflow_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None):
        taps = _bf_forward_taps(backflow_net, X, spin, track_grad=False)
        D = taps["dx"].reshape(X.shape[0], -1)
        mu = D.mean(dim=0, keepdim=True)
        Dc = D - mu
        evals, U = torch.linalg.eigh((Dc.T @ Dc) / max(1, D.shape[0]-1) + 1e-12 * torch.eye(D.shape[1], device=D.device, dtype=D.dtype))
        evals = torch.flip(evals, dims=[0]); U = torch.flip(U, dims=[1])
        expvar = evals / (evals.sum() + 1e-12)
    ...
    @torch.no_grad()
    def _bf_pc_ablation_on_disp(backflow_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None,
                                ks: Iterable[int] = (1,2,3,4,6,8,12), n_random: int = 12):
        taps = _bf_forward_taps(backflow_net, X, spin, track_grad=False)
        D = taps["dx"].reshape(X.shape[0], -1)
        ...
        return dict(k=torch.tensor(list(ks), device=D.device), rel_err_pc=torch.tensor(rel_err_pc, device=D.device),
                    rel_err_rand=torch.tensor(rel_err_rand, device=D.device), pca_eff_rank=pca["eff_rank"])

    @torch.no_grad()
    def _bf_channel_ablation(backflow_net: nn.Module, X: torch.Tensor, spin: Optional[torch.Tensor] = None) -> Dict[str, float]:
        ...
        def _flow_from_msg_in(msg_in):
            ...
            if getattr(backflow_net, "out_bound", "tanh") == "tanh":
                dx_pre = torch.tanh(dx_pre)
            bf_scale = torch.nn.functional.softplus(backflow_net.bf_scale_raw) if hasattr(backflow_net, "bf_scale_raw") else torch.tensor(1.0, device=X.device, dtype=X.dtype)
            return dx_pre * bf_scale

        base_msg = torch.cat([xi, xj, r, r1, r2], dim=-1)
        abls = {}
        # xi
        msg = base_msg.clone(); msg[..., 0:d] = 0.0
        abls["xi"] = float(((_flow_from_msg_in(msg) - base).abs().mean()).item())
        # xj
        msg = base_msg.clone(); msg[..., d:2*d] = 0.0
        abls["xj"] = float(((_flow_from_msg_in(msg) - base).abs().mean()).item())
        # r_vec
        msg = base_msg.clone(); msg[..., 2*d:3*d] = 0.0
        abls["r_vec"] = float(((_flow_from_msg_in(msg) - base).abs().mean()).item())
        # r_norm
        off = 3*d
        msg = base_msg.clone(); msg[..., off:off+1] = 0.0
        abls["r_norm"] = float(((_flow_from_msg_in(msg) - base).abs().mean()).item())
        # r_sq
        msg = base_msg.clone(); msg[..., off+1:off+2] = 0.0
        abls["r_sq"] = float(((_flow_from_msg_in(msg) - base).abs().mean()).item())
        return abls

    @torch.no_grad()
    def _bf_near_field_concentration(backflow_net: nn.Module, X: torch.Tensor, omega: float, spin: Optional[torch.Tensor] = None,
                                     qs: Iterable[float] = (0.01,0.05,0.10)) -> Dict[str, list]:
        taps = _bf_forward_taps(backflow_net, X, spin, track_grad=False)
        DX = taps["dx"]
    ...
    def _bf_linear_probes_on_disp(backflow_net, X, omega: float, spin=None, top_k: int = 3):
        ...
        return dict(R2=torch.tensor(R2, device=D.device, dtype=D.dtype), target_names=phys["names"], U_topk=Uk)
    ...
    psi_log_fn_nb = make_psi_log_fn(psi_fn, f_net, C_occ, backflow_net=None,        spin=None, params=params)
    psi_log_fn_bf = make_psi_log_fn(psi_fn, f_net, C_occ, backflow_net=backflow_net, spin=None, params=params) if backflow_net is not None else None

    X_nb, acc_burn_nb, acc_mix_nb = sample_psi2_batch(
        psi_log_fn_nb, B=B, N=f_net.n_particles, d=f_net.d, omega=f_net.omega,
        device=device, dtype=dtype, method="rw", step_sigma=0.20 * std,
        burn_in=100, mix_steps=30, autotune=True, target_accept=0.60
    )
    ...
    if backflow_net is not None:
        X_bf, acc_burn_bf, acc_mix_bf = sample_psi2_batch(
            psi_log_fn_bf, B=B, N=f_net.n_particles, d=f_net.d, omega=f_net.omega,
            device=device, dtype=dtype, method="rw", step_sigma=0.20 * std,
            burn_in=100, mix_steps=30, autotune=True, target_accept=0.60
        )
    ...
    Zc_nb = Z_nb - pca_nb["mu"]; Uk1_nb = pca_nb["U"][:, :1]
    ...
    if backflow_net is not None:
        ...
        res_pc_bf = pc_projection_ablation(f_net, X_bf, None, ks=(1,2,3,4,6,8,12), n_random=12)
        ...
        C_dZ = (dZc.T @ dZc) / (max(1, dZc.shape[0] - 1)) + 1e-12 * torch.eye(dZc.shape[1], device=dZ.device, dtype=dZ.dtype)
        ...
        cos_pc1 = float(((Uk1_nb.squeeze() @ Uk1_bf.squeeze()) /
                        (Uk1_nb.squeeze().norm() * Uk1_bf.squeeze().norm() + 1e-12)).item())
        ...
        if mask_fin.any():
            corr_dE_base = float(torch.corrcoef(torch.stack([delta_E_on_bf[mask_fin], base_bf[mask_fin]]))[0, 1])
            corr_dE_cusp = float(torch.corrcoef(torch.stack([delta_E_on_bf[mask_fin], cusp_bf[mask_fin]]))[0, 1])
        else:
            corr_dE_base = float("nan")
            corr_dE_cusp = float("nan")

        # Near-field concentration of |ΔE|
        def _near_field_delta_share(f_net: nn.Module, X: torch.Tensor, delta: torch.Tensor,
                                    qs: Iterable[float], min_count: int) -> Dict[str, list]:
            ...
            for q in qs:
                ...
                if cnt < min_count:
                    ...
                    mask = torch.zeros_like(rmin, dtype=torch.bool); mask[idx] = True; cnt = k
                share = float((torch.nanmean(delta[mask].abs()).item()) / base)
                rows.append(dict(q=float(q), share=share, count=cnt, frac=float(cnt / B)))
            return dict(rows=rows)

        nf_delta = _near_field_delta_share(f_net, X_bf, delta_E_on_bf, qs=near_qs, min_count=min_qcount)
        ...
        def _feature_cov_cond(f_net: nn.Module, X: torch.Tensor) -> float:
            ...
            C = (Zc.T @ Zc) / (max(1, Zc.shape[0] - 1)) + 1e-10 * torch.eye(Zc.shape[1], device=Z.device, dtype=Z.dtype)
        ...
        bf_pcab    = _bf_pc_ablation_on_disp(bf_net, X_bf, spin=None, ks=(1,2,3,4,6,8,12), n_random=12)
        bf_chan    = _bf_channel_ablation(bf_net, X_bf, spin=None)
        bf_probe   = _bf_linear_probes_on_disp(bf_net, X_bf, omega=float(f_net.omega), spin=None, top_k=3)
        bf_nf_dx2  = _bf_near_field_concentration(bf_net, X_bf, omega=float(f_net.omega), spin=None, qs=(0.01,0.05,0.10))

        backflow_report = dict(
            energies=dict(
                mean_noBF=mu_nb, std_noBF=sd_nb, se_noBF=se_nb,
                mean_BF=mu_bf, std_BF=sd_bf, se_BF=se_bf,
                mean_delta=dmu, std_delta=dsd, se_delta=dse, nfinite_delta=dfin, ntotal_delta=dtot
            ),
            # PINN geometry comparisons (context)
            feature_eff_rank=dict(noBF=float(svd_nb["effective_rank"]), BF=float(svd_bf["effective_rank"])),
            pca_noBF=dict(
            ...
            pc_ablation=dict(
                noBF=dict(k=res_pc_nb["k"].tolist(),
                          rel_mae_pc=(res_pc_nb["mae_pc"]  / (float(f_net.rho(taps_nb["rho_in"]).std(unbiased=False)) + 1e-12)).detach().cpu().tolist(),
                          rel_mae_rand=(res_pc_nb["mae_rand"] / (float(f_net.rho(taps_nb["rho_in"]).std(unbiased=False)) + 1e-12)).detach().cpu().tolist(),
                          pca_eff_rank=float(res_pc_nb["pca_eff_rank"])),
                BF=dict(k=res_pc_bf["k"].tolist(),
                        rel_mae_pc=(res_pc_bf["mae_pc"] / (float(f_net.rho(taps_bf["rho_in"]).std(unbiased=False)) + 1e-12)).detach().cpu().tolist(),
                        rel_mae_rand=(res_pc_bf["mae_rand"] / (float(f_net.rho(taps_bf["rho_in"]).std(unbiased=False)) + 1e-12)).detach().cpu().tolist(),
                        pca_eff_rank=float(res_pc_bf["pca_eff_rank"]))
            ),
            energy_feature_corr=dict(
                noBF=dict(corr_features=energy_feature_correlations(taps_nb, E_L_nb.view(-1))["corr_features"].detach().cpu().tolist(),
                          corr_out_base=float(energy_feature_correlations(taps_nb, E_L_nb.view(-1))["corr_out_base"]),
                          corr_cusp=float(energy_feature_correlations(taps_nb, E_L_nb.view(-1))["corr_cusp"])),
                BF=dict(corr_features=energy_feature_correlations(taps_bf, E_L_bf.view(-1))["corr_features"].detach().cpu().tolist(),
                        corr_out_base=float(energy_feature_correlations(taps_bf, E_L_bf.view(-1))["corr_out_base"]),
                        corr_cusp=float(energy_feature_correlations(taps_bf, E_L_bf.view(-1))["corr_cusp"]))
            ),
            ...
            bf_network=dict(
                ranks=bf_ranks,
                pca_disp=dict(eff_rank=bf_pca["eff_rank"],
                              expvar_top8=bf_pca["expvar"][:8].detach().cpu().tolist()),
                pc_ablation=dict(k=bf_pcab["k"].tolist(),
                ...
                probes=dict(names=[str(n) for n in bf_probe["target_names"]],
    ...
    print(f"Branch Δout (no-BF): ψ={ablation_nb['psi']:.3f}, φ={ablation_nb['phi']:.3f}, extras={ablation_nb['extras']:.3f}")
    print(f"Means (out, cusp, base | no-BF): {cusp_means_nb}")
    print(f"Eff. rank (features | no-BF) = {float(svd_nb['effective_rank']):.3f} | Eff. rank (ρ_in | no-BF) = {float(res_pc_nb['pca_eff_rank']):.3f}")
    print("PC ablation rel-MAE (PC vs random | no-BF):")
    for k, r_pc, r_rd in zip(res_pc_nb["k"].tolist(), rel_pc_nb, rel_rand_nb):
        print(f"  k={k:>2}  PC={r_pc:.4e}  rand={r_rd:.4e}")
    print("PC probes R^2 (no-BF):", {n: float(r) for n, r in zip(probe_nb['target_names'], probe_nb['R2'].tolist())})
    print(f"corr(head, head@PC1 | no-BF) = {corr_full_pc1_nb:.6f}")
    print("PC1 block power shares (no-BF):", {k: round(v, 3) for k, v in pc1_block_power_nb.items()})
    for row in nf_nb["rows"]:
        print(f"[Near-field q | no-BF] q={row['q']:.0%}  share={row['share']:.3f}  count={row['count']}  frac={row['frac']:.5f}")

    if backflow_net is not None:
        print("\n--- Backflow vs No-Backflow (PINN context) ---")
        print(f"ΔE (mean±se) on BF samples: {backflow_report['energies']['mean_delta']:.6f} ± {backflow_report['energies']['se_delta']:.6f} "
        ...
        for row in backflow_report["near_field_delta"]:
            print(f"[Near-field ΔE | BF] q={row['q']:.0%}  share={row['share']:.3f}  count={row['count']}  frac={row['frac']:.5f}")

        # --- BF network proper ---
        bf = backflow_report["bf_network"]
        print("\n--- BackflowNet (Δx/messages) ---")
        print(f"Eff-rank(Δx)={bf['ranks']['disp_eff_rank']:.3f}  PR(Δx)={bf['ranks']['disp_pr_rank']:.3f} | "
              f"Eff-rank(msg)={bf['ranks']['msg_eff_rank']:.3f}  PR(msg)={bf['ranks']['msg_pr_rank']:.3f}")
        print(f"PCA(Δx) eff-rank={backflow_report['bf_network']['pca_disp']['eff_rank']:.3f} "
              f"| expvar top-4={ [round(float(x),6) for x in backflow_report['bf_network']['pca_disp']['expvar_top8'][:4]] }")
        print("PC ablation rel-||·|| (Δx):")
        for k, r_pc, r_rd in zip(bf["pc_ablation"]["k"], bf["pc_ablation"]["rel_err_pc"], bf["pc_ablation"]["rel_err_rand"]):
            print(f"  k={int(k):>2}  PC={r_pc:.4e}  rand={r_rd:.4e}")
        print("Linear probes R^2 (Δx PCs → phys):", {n: float(r) for n, r in zip(bf['probes']['names'], bf['probes']['R2'])})
        print("Input-channel ablation Δx-change:", {k: round(float(v), 4) for k, v in bf["channel_ablation"].items()})
        for row in bf["near_field_dx2"]:
            print(f"[Near-field ||Δx||²] q={row['q']:.0%}  share={row['share']:.3f}  count={row['count']}  frac={row['frac']:.5f}")
    ...
    out = dict(
        ...
        probes_noBF=dict(names=probe_nb["target_names"], R2=[float(x) for x in probe_nb["R2"].tolist()]),
    ...
    if backflow_net is not None:
        out["energy"]["withBF"] = dict(mean=mu_bf, std=sd_bf, se=se_bf, nfinite=nfin_bf, ntotal=ntot_bf)
