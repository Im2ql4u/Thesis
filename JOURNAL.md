...
### [2026-09-12] — Wigner diagnostics: the bond-order null is the pipeline's null; Delta-cos carries tight intervals

**Motivation:** Supervisor review asked (i) for the random-angle null of |Phi_n| to be generated through the same pipeline instead of the asymptotic 0.886/sqrt(n), and (ii) for bootstrap intervals and analysis-choice sensitivity on the held-out polygon Delta-cos, whose N=20 values are only 0.003–0.010.
**Method:** The shell-summary and polygon-fit code had been lost with `src/Analysis2.ipynb` (deleted 2026-01-10); recovered from the cluster trash and restored as `src/wigner_structure.py`. A sequential pass with the original shared generator (seed 7) reproduces all 15 published Delta-cos to six decimals. Per cell: bond order on the strided frames with a 50-block bootstrap; pipeline null = same frames, radii kept, all angles redrawn uniformly, identical rank-shell assignment; Delta-cos block bootstrap (40 blocks per half, refit + fresh model seed, 200 replicates); sensitivity to bins (120/480), split (swapped/interleaved), jitter scale (0.75/1.25) and shell threshold (z_thr 1.10/1.80 around the pipeline's 1.45).
**Results:** Shells are assigned by radial rank only, so the exact i.i.d. null (Kluyver integral) equals the pipeline null — confirmed in every cell. Ratio errors 0.002–0.007. N=6 strong confinement sits resolvably below the null (0.989±0.002 at w=1, 0.990±0.002 at 0.5); N=12/20 within 0.5% through w=0.1; 1.16–1.63 at 1e-3. Delta-cos 95% intervals exclude zero everywhere (N=20 w=1: [0.0027,0.0030]); growth with 1/omega resolved (N=6 all disjoint; N=12/20 only w=1/0.5 touch). Bins and split: <=0.0003. Jitter +-25%: +34%/−22% in size, sign and monotone trend kept. Threshold: only matters where it produces spurious partitions ((1,3,16) at N=20 w=0.5/0.1; (1,3,8) at N=12 w=0.01), −10–16%.
**Interpretation:** The sub-null N=6 value is physics — a correlation hole along the ring (negative cross terms in <|Phi_n|^2>) without n-fold order — not a pipeline artefact. The staged-crossover claims now have quantified support; Delta-cos magnitude is model-dependent, its sign and trend are not.
**Caveats:** The bootstrap treats contiguous blocks of MCMC frames as independent; with 40–50 blocks per half this is standard but not verified by an autocorrelation scan. The jitter model is Gaussian throughout.
**Output reference:** `results/analysis/2026-09-12_wigner_null_bootstrap/` (per-cell JSON incl. replicates, reproduce.json), `scripts/wigner_null_bootstrap.py`.
**Next question:** Whether the single-particle radial diagnostics (Table 6.3) should be recomputed on the same December runs as the shell-resolved ones — they come from an earlier pass on different runs.

### [2026-09-12] — Independent blocked MCMC re-evaluation of every N=6/N=12 collocation energy

**Motivation:** Supervisor review questioned the "+0.28%" collocation headline on the grounds that it was scored by an importance-sampling estimator that the thesis itself calls unreliable at k-hat > 1.
**Method:** Found that the premise was a mis-description: the campaign's reported energies were a fresh 30k-sample *Metropolis* evaluation (`evaluate_energy_vmc`, persistent chain), not IS. Regardless, re-evaluated all 30 campaign checkpoints (`outputs/2026-04-11_phase1_corrected_matrix`, post-IS-fix) and the three post-fix N=12 continuations with `scripts/eval_collocation_blocked.py`: trainer's own loader in eval-only mode, evaluator swapped for 4 independently seeded persistent Metropolis chains x 200 sweeps x 512 walkers (2000 burn-in steps, thin 10), blocking on per-sweep means, SE = max(blocked, between-chain). Legacy exp(−r) cusp inferred for checkpoints predating the May cusp change (the trainer default would have scored a different wavefunction at low omega).
**Results:** Robust recipe, weak confinement confirmed: w=0.1 +0.293±0.009%, 0.01 +0.244±0.020%, 1e-3 +0.282±0.059% (logged means 0.286/0.269/0.281). Baseline 1e-3 +5.18% (logged 5.27). Strong confinement: every run at w>=0.5 is +0.07–0.10% — the published best +0.013% (w=1) and +0.002% (w=0.5) re-evaluate at +0.081±0.006% and +0.103±0.012%. Across all 30 runs the checkpoint-selecting probe is optimistic by 0.073% on average (26/30), while the original final evaluation is unbiased on average (+0.001%). Blocked SE ~1.5x naive; between-chain spread ~1.5x within-chain. N=12 post-fix w=1: +0.031±0.003%.
**Interpretation:** The +0.28% claim stands, now on the evidence the reviewer asked for. The "statistical consistency with DMC at strong confinement" claim does not: the best-of-three on a noisy single evaluation was the lucky tail. The thesis's "Step 4 / IS evaluation gap" section had the conclusion backwards (probe lucky, not evaluation noisy) and was rewritten as selection bias.
**Caveats:** Four chains per checkpoint give a noisy between-chain SE; the conservative max() is used. At 1e-3 the between-chain spread exceeds the blocked error, consistent with slow sector mixing ((1,5)/(0,6)) within chains.
**Output reference:** `results/analysis/2026-09-12_colloc_blocked_eval/` (33 JSON with per-sweep series, logs, run_all.sh), cluster `~/Thesis_repo`.
**Next question:** None needed for the thesis; for future work, the probe-selection bias argues for selecting on a held-out probe or on Var(E_L) rather than on the minimum probe energy.
