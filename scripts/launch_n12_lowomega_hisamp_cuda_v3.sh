...
RUN_ID="${RUN_ID:-2026-04-19_n12_lowomega_hisamp_cuda_v3}"
...
run_case() {
  ...
  local tag="n12_lowomega_${variant}_cuda_s42"
  local mpl_dir="/tmp/matplotlib-n12-${variant}-gpu${gpu}"
  mkdir -p "$mpl_dir"
  ...
  MPLCONFIGDIR="$mpl_dir" CUDA_VISIBLE_DEVICES="$gpu" PYTHONUNBUFFERED=1 timeout "$WALL" \
    python3 src/run_weak_form.py "${args[@]}" 2>&1 | tee -a "$log_file"
  local rc=${PIPESTATUS[0]}
  echo "[$(date '+%F %T')] END   gpu=${gpu} variant=${variant} rc=${rc}" | tee -a "$log_file"
  return "$rc"
}

echo "[$(date '+%F %T')] Launching N=12 low-omega CUDA high-sample sweep: ${RUN_ID}" | tee -a "${OUT_DIR}/launcher.log"
for i in "${!GPUS[@]}"; do
  gpu="${GPUS[$i]}"
  variant="${VARIANTS[$i]}"
  n_coll="${N_COLL[$i]}"
...
wait
echo "[$(date '+%F %T')] All N=12 CUDA high-sample runs finished" | tee -a "${OUT_DIR}/launcher.log"
