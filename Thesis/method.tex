...
requires a determinant. The Coulomb divergence admits an analytic cancellation, so it is
imposed rather than learned. Second derivatives enter the loss, so the features fed to the
...
\subsection{Shared output, scaling, and initialisation}
...
$\Delta_\beta\!\approx\!0$ initially. Empirically this parameterisation stabilises the
...
To avoid a spurious centre-of-mass (COM) drift, we project the flow to zero mean:
\begin{equation}
  \Delta_\beta \;\leftarrow\; \Delta_\beta \;-\; \frac{1}{N}\sum_{i=1}^N \Delta_{\beta,i}.
  \label{eq:bf-com}
\end{equation}
The messages are built from the relative vectors $\mathbf r_{ij}$ and the zero-mean projection removes the spurious centre-of-mass drift; as noted under the design goals, the map is not translation-invariant or rotation-equivariant by construction (the displacement head reads a $d$-vector from the absolute coordinates).%
\footnote{Implementation note: the mean is taken over particles \emph{within each
configuration}, not across configurations in the batch, so the constraint removes
centre-of-mass motion configuration by configuration. It is a one-liner and removes the
small residual COM drift seen in ablations.}

The two backflow variants apply this projection at \emph{different points}, and the
difference is invisible in the wavefunction but not in the diagnostics. The copresheaf
network (\S\ref{subsec:ctnn-backflow}) applies Eq.~\eqref{eq:bf-com} inside its forward
pass, so the field it returns is already COM-free; the conventional network returns the raw
displacement and the projection is applied by the caller when $\tilde R'$ is formed. The
shifted coordinates are therefore identical either way, but any diagnostic evaluated on a
network's \emph{returned} field sees a COM component in one case and not the other. This is
what puts the two columns of the backflow-rank measurement in \S\ref{sec:kernel-q1b} under
ceilings that differ by two, and it is stated there again where it bites.
...
This ``Feynman--Cohen style''~\cite{FeynmanCohen1956-Backflow} design targets fixed-node
error directly while keeping the amplitude model simple and smooth; the same idea in its
modern quantum-Monte-Carlo form is due to Holzmann and
Ceperley~\cite{HolzmannCeperley2003-Backflow}.

\subsection{Regularization, initialisation, and cost}
\label{sec:bf-init}
We found the following defaults robust across $(N,\omega)$:
(i) $\varepsilon_{\rm bf}=10^{-6}$ (trap units) for the backflow soft norm of
\eqref{eq:bf-message}, fixed rather than tuned. It is a numerical guard, not a modelling
choice: $\sqrt{\tilde r^2+\varepsilon_{\rm bf}^2}$ differs from $\tilde r$ by less than
$\varepsilon_{\rm bf}$ everywhere, so it changes no value the network can resolve and
exists only to keep $\nabla\tilde r$ finite at exact coincidence, which
$\nabla\sqrt{\tilde r^2}$ is not. It should not be read as a short-range physical scale:
that role belongs to the pair-feature softening $\varepsilon\in[0.15,0.30]$ of
\eqref{eq:safe-feats}, four to five orders of magnitude larger, which is what actually sets
where the correlator stops resolving structure;
(ii) bounded output $\tanh$ with a learnable scale $s_\beta$ initialised in $[0.02,0.10]$;
(iii) optional gate $\chi_{\rm bf}$ with $r_g\approx 0.3$ (trap units).
Zero-initialising the last layer of $\psi_\beta$ makes the ansatz start exactly at the Slater nodes and improves optimiser stability.
...
subtracting the per-configuration mean over particles of the predicted $\Delta_\beta$ before forming $\tilde R'$.

\paragraph{Summary.}
Backflow provides nodal freedom by moving the Slater coordinates, while also modifying the determinant amplitude inside each nodal pocket. With mollified distances, optional short-range gates/attention, zero-mean projection, and bounded outputs, it integrates cleanly with the analytic cusp and smooth NQS correlator, designed to reduce nodal error while controlling local-energy variance.
...
Chapter~\ref{ch:kernel} stand on different footings in this respect and it is worth saying
...
\caption[Architecture and training hyperparameters]{Architecture and training
hyperparameters for two of the three programmes of p.~\pageref{ch:results-map}; the
collocation models are given in \S\ref{subsec:colloc-families}. Widths are hidden dimensions
and ``rounds'' are message-passing steps ($n_\downarrow{+}n_\uparrow$ for the V-cycle). The
two sets of counts are \emph{different networks} and are never compared with one another. The
architecture-study widths are those of the controlled comparison and of the $\Var(E_L)$
measurement in Table~\ref{tab:varEL}; within it the correlator variants are not
capacity-matched, which is why \S\ref{sec:kernel-q1a} also reports a capacity-matched
$89.4$k separable network and the full $20$k--$164$k ladder. The backflow comparison has no
separate tier at all: it is run at the production widths, where the two variants differ by a
factor of $3.6$ in size, for the reason given above. The physics-production widths
are those carrying the ground-state energies and the structural results of
Chapter~\ref{ch:results}, where the correlator is the separable network throughout (code:
\texttt{PINN}) and only the backflow is varied.}
\label{tab:hyperparams}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}l l l r l r@{}}
\hline
 & & \multicolumn{2}{c}{Architecture study (\S\ref{sec:kernel-q1})} & \multicolumn{2}{c}{Physics production (\S\ref{sec:energy-overview})} \\
Component & Variant & setting & params & setting & params \\
\hline
Correlator & separable        & pair-MLP $64\times4$, out $32$                  & $47.7$k & width $128$, $2$ layers & $54$k \\
           & message-passing  & node/edge $32$, bottleneck $16$, $2{+}2$ rounds & $79.8$k & ---                     & --- \\
Backflow   & message-passing  & \emph{as production}                            & $182$k  & msg $128$, node $128\times3$ & $182$k \\
           & conventional     & \emph{as production}                            & $51$k   & msg $128$, node $128\times3$ & $51$k \\
Readout    & (correlator)     & hidden $64$, $2$--$3$ layers, SiLU              & ---     & ---                     & --- \\
\hline
\multicolumn{5}{@{}l}{Optimiser: Adam, $\mathrm{lr}=5\times10^{-4}$ (backflow),
  $5\times10^{-5}$ (Jastrow); CG-SR with Tikhonov damping $\lambda$,} \\
\multicolumn{5}{@{}l}{\quad trust-region cap and CG restarts. Collocation batch
  $n_{\rm coll}\sim3\times10^3$, oversampling $m=8$--$16$.} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{CG-SR}: the same step solved matrix-free by preconditioned conjugate
    ...
    anisotropic but still statistically resolved, and becomes unstable at weak confinement,
    where the sampling that builds $S$ collapses and the empirical metric can no longer be
    estimated (\S\ref{sec:kernel-cond}).
\end{itemize}
...
problem~\cite{ChenHeyl2024-minSR,Rende2024-KernelSR}. In the $N\le6$ comparisons of \S\ref{sec:kernel-q2}, Adam and SR reach indistinguishable
energies under $|\Psi|^2$ sampling; they differ in conditioning and, decisively, in the
collocation regime. Whether that equivalence persists at larger $N$ is not established
there.
...
collapses. Angular crystallisation of the Wigner ring sets the next limit and is addressed
there by a Dirichlet model of the angular gaps; what bounds the route below
$\omega=0.0035$ is a separate, non-sampling failure of transfer between confinements. Difficult
...
networks of \S\ref{sec:hyperparameters}, because this programme is limited by gradient noise
and memory rather than by capacity.
...
(single-particle orbital values) and edge features (soft-core pair distances). Fastest to
train and the most robust; used both standalone and as the initialiser for backflow runs.
...
Two training routes have now been described and they are used for different purposes
throughout the thesis, so it is worth setting them side by side before the diagnostics
chapter. Both start from the same ansatz and end at an energy evaluated under
...
checkpoints sits outside the update loop entirely. The steps of the left-hand route, which
carries the ground-state energies of \S\ref{sec:energy-overview}, are:
...
\end{equation}
and the ablations below re-evaluate $\rho_\theta$ on modified inputs rather than manipulating
a weight vector.
...
unphysical. What the comparison licenses is a relative statement between two architectures
...
is the smallest \emph{sample-supported} eigenvalue rather than an intrinsic one; the value of
$\kappa$ therefore depends on $B$ and is a property of the empirical metric the optimiser
actually sees, not of the variational manifold. That is the relevant object for the question
we ask of it---whether preconditioning by $S^{-1}$ helps---but it means $\kappa$ may not be
...
across a node differently. The direction of the bound matters, and it is worth being explicit about which claim it
protects. The modulus overlap is an \emph{upper} bound on the signed one, so a reported
$\mathcal{S}^2\to0$ is conservative---the states are at least that different---while a
reported $\mathcal{S}^2\approx1$ is \emph{not} by itself evidence that they agree in sign.
For the $\mathcal{S}^2\approx1$ readings the weight is carried by the structural argument
above (shared Slater reference, strictly positive correlator), not by the bound. These quantities are properties of the state and so are unaffected by how correlation is
divided between the Jastrow and the backflow. The energetic price of ablating a component is
\emph{not} in that class: two parameterisations that divide correlation differently will
report different ablation costs, so it is a reproducible statement about a fixed model and
training protocol rather than a state invariant (\S\ref{sec:synthesis}). The architectural
conclusions of Chapter~\ref{ch:kernel} rest on the state-level quantities first and on
ablation as a controlled intervention second, and on the gauge-dependent component ranks of
the previous section not at all.
