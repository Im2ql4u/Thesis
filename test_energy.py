...
import sys, math, gc, torch, numpy as np
...
bf = CTNNBackflowNet(d=2, msg_hidden=128, msg_layers=2, hidden=128,
                     layers=3, act="silu", use_spin=True, omega=OMEGA)
...
metrics = evaluate_energy_vmc(
    f_net, C_occ, psi_fn=psi_fn, backflow_net=bf,
    compute_coulomb_interaction=compute_coulomb_interaction,
    params=params,
    persistent=True, n_samples=20_000, batch_size=1024,
    sampler_step_sigma=0.2, sampler_target_accept=0.5,
    sampler_adapt_lr=0.05, sampler_burn_in=200, sampler_thin=20,
    lap_mode="exact", lap_probes=32, fd_eps=3e-3,
    assume_omega_invariant=True, progress=True,
)
