...
\chapter{Why the method works: geometry, optimisation, and sampling}
\label{ch:kernel}

The previous chapter showed that the ansatz works. It reaches reference quality wherever
a reference exists, the states it produces are genuine Wigner molecules, and it holds
that structure on a handful of internal directions. Why it works is a separate question,
and a harder one.

Two networks of the same size, trained the same way, landing on the same energy, need
not be the same object. A natural-gradient step that is decisive in one training regime
does nothing in another. An importance-sampled objective reproduces a Markov chain's
answer in one corner of the phase diagram and parts company with it in the next. These
look like three unrelated puzzles. They are one, and it is geometric.
...
neural tangent kernel $K = OO^{\!\top}$. Architecture sets the shape of that geometry.
The optimiser inverts it. The sampling measure decides whether it can be estimated at
all. Read in that order, the three puzzles become one story.
...
Three constraints hold across the chapter. Architectures are compared on a
\emph{common probe set}---the same configurations for every model, never each model's own
$|\Psi|^2$. Backflow comparisons are seeded and held at reference quality. And below
$\omega=0.1$ every relative error is measured against our own converged variational
energies, which makes it a statement about internal consistency and not about accuracy.
...
\paragraph{What follows, in short.}
A message-passing correlator reaches the ground state in half as many effective tangent
directions as a separable network of matched or larger capacity ($1.40\pm0.17$ against
$3.25\pm0.03$ at $N{=}6$, $\omega{=}1$, seeded, $\sim\!11\sigma$), and those directions
are the collective modes of the Hamiltonian itself---breathing, correlation hole,
relative excitation---to $R^2=1.00$ at the exact two-electron anchor and
$R^2\!\gtrsim\!0.93$ at $N{=}6$. The separable network spends nearly half its tangent
budget where no compact physical dictionary can find it, and carries a higher
$\Var(E_L)$ for it at every confinement, while matching the energy to four figures.

In the nodes the same channel shows up as a collapse averted rather than a contribution
made. Toward the Wigner crystal a conventional backflow falls onto a single collective
mode---rank $10\!\to\!1$ at $N{=}6$, $22\!\to\!1$ at $N{=}12$---and loses half a percent
of the energy, while the copresheaf field stays near its $2N$ ceiling and holds
$\sim\!0.02\%$. Deleting the copresheaf backflow outright at that confinement costs
$0.017\%$. So the message-passing backflow earns its place there not by what it adds but
by what it declines to do: a backflow collapsed onto one mode does not sit idle, it
drags the nodes with it. Correlator and backflow give way at the same $\omega$, and a
better correlator postpones both.

The optimiser and the sampler follow from the same geometry. Stochastic reconfiguration
whitens the tangent kernel, so its value tracks $\kappa(S)$: negligible where
$|\Psi|^2$ sampling has already conditioned the metric, decisive on the collocation
objective, harmful once the sampling that builds the metric has collapsed. And a
Markov-chain-free objective reaches the \emph{same state} as variational Monte Carlo
while its importance sampling stays healthy, then diverges from it when the sampling
fails---though a proposal built from the Wigner shell itself carries it to
$\omega=0.0035$ before the wall.

One caveat runs underneath and is itself a result. Quantities living on a single
component---backflow rank, displacement magnitude---are gauge-like, fixed by the training
route rather than by the state. The invariants are energy, local-energy variance, and
ablation, with state overlap carrying the comparison between training paradigms.
\section{Message passing reorganises the variational geometry}
\label{sec:kernel-q1}

What does a message-passing network learn that a separable one cannot? The answer is
the same in both learnable objects, and it is a statement about the tangent kernel. In
the correlator, message passing \emph{compresses}: the same ground state is reached along
fewer effective directions. In the backflow it prevents a \emph{collapse}: the
displacement field keeps its rank where a separable one loses it. We take them in turn.

\subsection{The correlator: compression onto physical directions}
...
\subsection{The backflow: a collapse averted}
...
\subsection{One bottleneck, two channels}
...
% =====================================================================
% =====================================================================
\section{Conditioning: what the optimiser can do depends on the measure}
...
\subsection{Whitening, and where it pays}
...
So the verdict is neither ``SR wins'' nor ``SR ties''. \textbf{SR matters when the
tangent metric is ill-conditioned and still estimable.} Its value is a
...
% =====================================================================
\subsection{The measure sets the conditioning, the variance, and the reach}

Between $|\Psi|^2$-sampled VMC and importance-sampled collocation the wavefunction family
is identical; only the measure that defines the loss changes. Three consequences follow,
...
(Table~\ref{tab:collocation}). What follows is why that works, where it stops, and what
the stopping tells us.
\subsection{The objective, and why it is stable}
...
picture (\S\ref{sec:kernel-cond}): where the metric is
...
%% ═══════════════════════════════════════════════════════════════════════
\paragraph{Negative: Langevin proposal refinement, revisited.}
...
\paragraph{The three-regime optimiser boundary.}
...
\S\ref{sec:kernel-cond}. Where importance-sampling collapse makes the empirical
...
\section{A proposal built from the physics, and the wall beyond it}
...
% =====================================================================
% =====================================================================
\section{A gauge caveat, and what survives it}
\label{sec:synthesis}

One thing has to be said plainly before any of this is carried forward, because it
governs which of the numbers above can be leaned on.

Component-level quantities---the backflow's rank, its displacement magnitude
$|\Delta\mathbf{x}|$, the per-component $d_{\mathrm{eff}}$---are \textbf{gauge-like}. The
Jastrow and the backflow are substitutes. How correlation is divided between them is a
nearly flat, path-dependent direction of the loss, so those quantities record the route
the training took and not the state it reached. This is not a hypothetical worry: every
claim that reversed during this project was of exactly that kind---the
$d_{\mathrm{eff}}$ inversion at the crystal that did not survive seeding, a mislabelled
backflow, an artefactual rank collapse produced by training both components jointly
from scratch.

Four measurements are invariant to that gauge, and the conclusions of this chapter rest
only on them. \emph{Energy} and \emph{ablation}---delete a component and price it---carry
the architecture comparison. $\Var(E_L)$ separates the two families where energy cannot
and where $d_{\mathrm{eff}}$ has stopped trying: at matched protocol and production
capacity it is lower for the message-passing ansatz at every confinement, by factors of
two to seven (Table~\ref{tab:varEL}), and seed-cleanly so at the crystal. And \emph{state
overlap} is what makes the comparison between training paradigms a claim about states
rather than about numbers that happen to agree.

Rank is not on that list. It appears throughout this chapter because it is the clearest
\emph{signature} of the mechanism---a field collapsing onto one mode is a vivid thing to
watch---but the finding it signals is carried by the half-percent of energy that goes
with it, not by the rank itself.
