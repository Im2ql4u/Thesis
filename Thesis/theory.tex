...
\chapter[The many-body problem]{The many-body problem and its numerical difficulty}
\label{sec:theory}
\endgroup
...
not a formality: choosing it to match the physics---harmonic-oscillator orbitals for confined
charges, plane waves for periodic systems---is what makes a representation compact, and it is
...
annihilation operators rather than by the wavefunction, is the usual language for all three;
our own ansatz is first-quantised throughout, so it appears here as vocabulary rather than
machinery.

Our determinant is built from harmonic-oscillator orbitals rather than self-consistent
...
spin-adapted eigenstate of $S^2$ is in general a sum of spatial--spin products rather than a
single one; what we use computationally is the weaker and sufficient statement that with the
...
so the objective reads the network's second derivatives rather than its values, and reads
...
count, they permit architectures, optimisers and training paradigms to be compared on a
...
The standard schemes---Xavier/Glorot, scaling weight variance as $1/(n_{\rm in}+n_{\rm
out})$, and Kaiming/He, scaling as $2/n_{\rm in}$ for rectified units---are designed to keep
...
(\S\ref{subsec:ctnn-backflow}, Eqs.~\eqref{eq:ctnn-edge}--\eqref{eq:ctnn-node}). The framing matters in two respects. First, the CTNN is \emph{one} concrete realisation of
...
The escapes are all statements about \emph{structure} rather than capacity: targets of
...
This is why the activation here is chosen for its second derivative rather than its values.
...
short-range form is built in rather than learned.
