...
range. A separable network of matched or larger capacity instead spends nearly half its
tangent budget on directions of \emph{no} physical response---a correlation deficit already
...
backflow's rank collapse set in at the \emph{same} confinement---one relational deficit, seen
in the amplitude and in the nodes. \textbf{(Q2)} SR whitens the tangent
...
physical-operator span as the \emph{non-physical tangent mass}. The DeepSet carries a large
...
(Fig.~\ref{fig:nonphys}). The reading is that a separable network can \emph{estimate} the
correlation energy---a small fraction of the total at strong confinement, so the energy looks
converged---without \emph{representing} correlation: it spends nearly half its tangent budget
on directions that correspond to no physical response. The deficiency is intrinsic and present
throughout; weak confinement merely removes the disguise, because there correlation \emph{is}
the energy. At $N=12$ the contrast is starker still: the CTNN's non-physical mass stays
...
\begin{figure}[htbp]\centering
  ...
  \caption{A limitation visible where the energy cannot see it. Non-physical tangent mass---the
  eigenvalue-weighted tangent variance outside the physical-operator span---versus $\omega$. At
  ...
  physically-uninterpretable directions against the CTNN's $6\%$. The excess is roughly constant
...
sides: when the amplitude model can no longer represent the correlation hole, the nodal
deformation can no longer stay high-rank, and a better correlator postpones \emph{both}. Q1a
and Q1b are one phenomenon.
