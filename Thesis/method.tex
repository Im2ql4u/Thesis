...
The ansatz is fixed first, because it determines the expressitivity of the model, and the gradient stability during training.
...
(%! do we not use regular harmonic oscillator eigenstates?)
...
This ``Feynman--Cohen style'' design targets fixed-node error directly while keeping the amplitude model simple and smooth [<citation>].
...
\section{gradient decoupling via REINFORCE}
...
precisely the regimes of greatest physical interest. (%!we must clarify, and missing sources  on dnesity thing)
...
\section{the optimizer regime boundary}
\label{sec:sr-stage}
...
Both share the same ansatz and the same local-energy evaluation. We always valdiate the results energy
of a trained wavefunction by running a separate MCMC chain with the final parameters,
 and we report the energy from that chain as the final result.
...
\paragraph{Stage II: stochastic reconfiguration & VMC.}
...
(%!maybe drop this or move to an appendix)
