...
acquire angular order inside them. How the network internally represents these states is a
question about the solver rather than about the physics, and is deferred to
...
\emph{conventional} backflow applies a single round of pairwise messages
aggregated per particle, whereas the \emph{message-passing} backflow maintains explicit node and
...
section has $\approx\!54\text{k}$ parameters, the conventional backflow adds
$\approx\!83\text{k}$ (total PINN+BF $\approx\!137\text{k}$), and the message-passing backflow adds
...
(Tables~\ref{tab:Residual} and \ref{tab:energies},
Fig.~\ref{fig:rel_error_vs_omega}). That is the result of this section, and everything
after it depends on it: it is what licenses treating the state as an instrument in
\S\ref{sec:wigner-molecule} rather than as a claim still under defence.

Two features of the tables carry more than the energies themselves. The first is
\emph{where the agreement holds}. A method that matches the reference at $\omega=1$ and
drifts at $\omega=0.1$ differs from one that holds across the whole benchmarked range, since
the physics changes character between those points while the architecture does not. The
second is \emph{where the tables stop}: below $\omega=0.1$ no reference of any kind exists
for $N\ge6$, so the values reported there are variational predictions rather than validated
ones, and that boundary is marked wherever it occurs rather than letting a uniformly
formatted table imply a uniform level of evidence.

One qualification attaches to the benchmark itself. Where a reference value exists it is
not only what we score against: it is also the target that residual pretraining anneals
toward (\S\ref{sec:opt-overview}). The agreement is therefore between a reference and an
\emph{independently evaluated variational upper bound on the same ansatz}, not between two
blind calculations---weaker than independence, and stronger than circularity. A variational
expectation under $|\Psi|^2$ is bounded below by $E_0$ whatever it was aimed at, so aiming
at a reference cannot pull the reported energy below what the ansatz can actually reach, and
aiming at one above that minimum would make the number worse rather than better.
\S\ref{sec:energy-discussion} takes the argument up properly.
...
For $N\in\{6,12,20\}$ the references are the fixed-node diffusion Monte Carlo energies of
  ...
  size, $N{=}20$ included.
  Parentheses denote $1\sigma$ on the last digit(s). The ``rel.\ dev.'' columns give
  $100\,(E-E_{\rm ref})/E_{\rm ref}$; because the $N\ge6$ references are fixed-node upper
  bounds, a negative entry is not necessarily an error, and the columns are labelled
  deviation rather than error for that reason (\S\ref{sec:energy-discussion}).
  \textbf{These are production energies and the two architectures are not capacity
  matched} ($\approx137$k against $\approx236$k parameters); the controlled architecture
  comparison is \S\ref{sec:kernel-q1}, and this table must not be read as an architecture
  ablation. A dash in the reference column marks
  ...
  \begin{threeparttable}
    \begin{tabular}{c|c|c|cc|cc}
      \toprule
      $N$ & $\omega$ & Reference & PINN+BF & rel.\ dev. & PINN+CTNN & rel.\ dev. \\
      \midrule
...
\begin{figure}[htbp]
  ...
  \caption[Relative deviation from the external references]{Relative deviation of PINN+BF
  ...
  kinds: the $N{=}2$ values are Taut's analytic two-electron
  solution~\cite{Taut1994-TwoElectrons}, and the $N{=}6$ and $N{=}12$ values are fixed-node
  diffusion Monte Carlo~\cite{Hogberget2013-DMC}. For $N{=}2$ the deviations are at the
  $10^{-3}\,\%$ level, comparable to the quoted precision of the analytic values; for
...
energies are tabulated there (Table~\ref{tab:collocation}) rather than here, because they
...
cloud first expands and its shells stiffen \emph{radially}---the width-to-radius ratio falls,
discrete occupancies emerge, the energy partition approaches classical force balance---while
the electrons remain angularly disordered within those shells. Angular order arrives later,
and abruptly: on every ring at every particle number the bond-orientational parameter sits
at its random-angle value down to $\omega=0.01$ and departs from it only in the last decade
of confinement. Radial localisation and angular crystallisation are not one transition, and
the diagnostics below are arranged to separate them.
...
\begin{figure*}[htbp]
  ...
  densities for $N=2,6,12$ at $\omega=1.0$ (top) and $\omega=0.001$ (bottom). As confinement
  weakens the densities broaden and develop pronounced \emph{radial} shell structure. Being
  lab-frame averages over all orientations of the molecule, they cannot show angular order:
  that requires the registered diagnostics of \S\ref{sec:wigner-angular}.}
...
\begin{figure}[htbp]
  ...
  for $N{=}20$ at strong and weak confinement. The weak-trap case shows strong radial
  broadening and clear multi-shell organisation. As above, this is radial evidence only.}
...
monotonically as the trap weakens at every particle number---at $N{=}12$ from $0.006$ at
$\omega=1$ to $0.026$ at $\omega=10^{-3}$, at $N{=}6$ from $0.012$ to $0.058$. Shell radii
alone account for most of the pair structure at strong confinement; polygonal order accounts
for a growing remainder as the molecule forms. This is the same staged transition the
bond-order ratios record below, measured on a different quantity and on data the model never
saw.
...
$r_1\approx45$), the registered angular order drops to
$\tilde C_{\mathrm{global}}\approx0.20$, and the outer-shell Lindemann rises to
$\approx0.57$. The wording matters here. At $\omega=10^{-3}$ the $(1,7,12)$ label describes a
...
\paragraph{What \S\ref{sec:wigner-molecule} establishes.} The crossover is staged. Radial
localisation---expansion, shell separation, falling width-to-radius ratio, classical force
balance---develops progressively from $\omega=1$ down. Angular order does not: on every ring
at every particle number the bond-order parameter sits at its random-angle null until
$\omega=10^{-2}$ and departs from it only in the final decade. Held-out polygon
reconstruction of $g(r)$ records the same staging on an independent quantity. What this does
\emph{not} establish is the total spin of these states, which we did not scan, nor that the
occupancy labels denote crystalline topologies outside the weakest-trap cases.
