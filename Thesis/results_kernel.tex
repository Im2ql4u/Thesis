...
\subsection{The remaining wall is the warm-start, not the proposal}
...
locates the obstacle not in the proposal but in the \emph{warm-start}. Changing $\omega$
rebuilds the $\omega$-dependent Slater determinant, which collapses the warm-started state
back to $\sim\!1\ell$ at every rung; re-expanding it to the shell by importance sampling is
inherently unstable, because the proposal either follows the collapse inward or, if held
out at the shell, loses ESS. A wide exploratory proposal \emph{can} drive the re-expansion
(the state was observed to expand to the classical shell at $\omega=0.0027$), but doing so
robustly at every rung is not a matter of proposal tuning. The clean resolution is an
$\omega$-robust warm-start---a coordinate rescaling that preserves the state's shape in
$\ell$-units across $\omega$, or an $\ell$-invariant parametrisation---which is a distinct
problem left to future work. The contribution here is concrete: a physics-informed proposal
that carries MCMC-free collocation from the origin-proposal wall down to $\omega=0.0035$,
and a precise account of the angular-crystallisation and warm-start effects that bound it.
