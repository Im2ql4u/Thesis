...
\chapter[Energies, structure, and representation]{Energies, Wigner-molecule structure, and the learned representation}
\label{ch:results}
\endgroup
...
analysis in Chapter~\ref{ch:kernel}. The architecture size is identical for all $N$. The correlator used for the energies in this
section has $\approx\!54\text{k}$ parameters, the conventional BackflowNet adds
$\approx\!83\text{k}$ (total PINN+BF $\approx\!137\text{k}$), and the CTNNBackflowNet adds
$\approx\!182\text{k}$ (total PINN+CTNN $\approx\!236\text{k}$). These are not the widths of
the controlled architecture comparison, which uses its own matched ladder
(Table~\ref{tab:hyperparams}); the two sets of counts are kept apart throughout and never
compared. All components are fully trainable;
...
Three parameter budgets are used for distinct purposes. These
...
These tables are the thesis's only contact with an external benchmark, and one thing has to
be said before they are read. Where a DMC value exists, it is not only what we score
against: it is also the target that residual pretraining anneals toward
(\S\ref{sec:opt-overview}). The agreement below is therefore between a reference and an
\emph{independently evaluated variational upper bound on the same ansatz}, not between two
blind calculations. That is weaker than independence and stronger than circularity: a
variational expectation under $|\Psi|^2$ is bounded below by $E_0$ whatever it was aimed at,
so aiming at a reference cannot pull the reported energy below what the ansatz can actually
reach, and aiming at one above that minimum would make the number worse rather than better.
\S\ref{sec:energy-discussion} takes the argument up properly. Two further features of these
tables carry more information than the energies themselves. The first is \emph{where the
agreement holds}: a method that matches DMC at $\omega=1$ and drifts at $\omega=0.1$ differs from one
...
\begin{table}[htbp]
  \centering
  \caption{Residual-based pretraining + SR tail. Reference energies and our PINN+BF
  and PINN+CTNN ground-state energies (Hartree) for selected $(N,\omega)$.
The reference column collects the best available benchmark for each size, and the two are
  \emph{not} of equal standing: fixed-node DMC from
  \cite{Pederiva2000-QD-DMC} (whose study spans $N\le13$) for $N\le12$, and, for $N=20$,
  which lies beyond the range of any published DMC study, the variational neural-network
  energies tabulated in the master's thesis of Daniel Haas~\cite{HaasHFQD}. The latter are
  themselves variational upper bounds rather than projected energies, so at $N{=}20$ a
  negative \%\,err means simply that our state is the lower of two variational estimates;
  the fixed-node argument of \S\ref{sec:energy-discussion} is needed only for $N\le12$.
  ...
  there, as no external benchmark is available to score against.}
  \label{tab:energies}
  \begin{threeparttable}
    \begin{tabular}{c|c|c|cc|cc}
      \toprule
      $N$ & $\omega$ & Reference & PINN+BF & \%\,err & PINN+CTNN & \%\,err \\
      \midrule
      ...
      6 & 0.001 & ---           & $0.140913(1)$ & --- & $\mathbf{0.140832(1)}$ & --- \\
        ...
        & 0.50 & $11.78484(6)$  & $11.7895(4)$  & $+0.0395$  & $\mathbf{11.7847(2)}$ & $-0.0012$ \\
      ...
      12 & 0.001 & ---          & $0.515823(3)$ & --- & $\mathbf{0.515365(4)}$ & --- \\
         ...
         & 0.10 & $12.26984(8)$ & $12.2731(2)$  & $+0.0266$ & $\mathbf{12.2743(2)}$ & $+0.0363$ \\
         & 0.50 & $39.1596(1)$  & $39.1786(8)$  & $+0.0485$ & $\mathbf{39.1604(3)}$ & $+0.0020$ \\
      ...
      20 & 0.001 & ---          & --- & --- & $\mathbf{1.293033(6)}$ & --- \\
         ...
         & 1.00 & $155.8822(1)^{\mathrm{H}}$  & ---   & --- & $\mathbf{155.8738(7)}$ & $-0.0054$ \\
      \bottomrule
    \end{tabular}
    \begin{tablenotes}\footnotesize
      \item[] Reference-energy provenance: values marked $^{\mathrm{H}}$ are variational
      neural-network energies from the master's thesis of Daniel Haas~\cite{HaasHFQD}
      ($N=20$), used because no published DMC value exists at this size and treated
      throughout as the weaker benchmark; all other reference energies are fixed-node
      diffusion Monte Carlo from Pederiva~\emph{et al.}~\cite{Pederiva2000-QD-DMC}
...
Read against Table~\ref{tab:energies}: at strong confinement the Markov-chain-free route is
inside DMC uncertainty for
...
Wherever an external check exists, the pattern is the same. For $N{=}2$ the ansatz reaches DMC
...
The typical radius $r_{\rm mode}$ grows as a power law, $r_{\rm mode}\!\sim\!\omega^{\alpha}$ with $\alpha\simeq-0.62$ at $N{=}2$---close to
...
respectively; here and below the quoted $\pm$ is the frame-to-frame standard deviation of
$|\Phi_m|$, not a standard error on its mean, which with $3\times10^4$ frames is smaller by
two orders of magnitude. These values must be read against a null: $n$ independent uniform
angles give $\langle|\Phi_n|\rangle=0.886/\sqrt n$, which is $0.40$ for $n{=}5$ and $0.36$
for $n{=}6$ (Appendix~\ref{app:structural-tables:shells}). At $\omega=0.01$ the measured
bond order is therefore barely above what randomly placed angles would give.
...
$|\Phi_6|=0.663\pm0.183$ (Lind$\,\approx\!0.205$)---now clearly above the random-angle
null, by factors of $1.5$ and $1.8$. This is the confinement at which angular order first
becomes measurable at any particle number
(Appendix~\ref{app:structural-tables:shells}); at $\omega\ge0.01$ the ratio to the null
is $1.00$--$1.10$ throughout.
...
essentially exactly (Fig.~\ref{fig:N12_w0001_decomposition}). That agreement is weaker
evidence than it looks: the sector histograms and the total are accumulated from the same
frames, so the recombination is close to exact by construction, and what it establishes is
that the sector bookkeeping is self-consistent rather than that the shell picture is right.

The test that can fail is generative and is reported in
Appendix~\ref{app:structural-tables:mixfit}. Fitting shell radii and widths on half the
frames, modelling each shell as a regular $n_s$-gon with a per-frame rotation and fitted
angular jitter, and scoring on the held-out half reproduces the pair distribution to cosine
similarity $0.987$--$0.999$ across the whole $(N,\omega)$ grid. An angle-randomised null
with the same radial distributions reaches only $0.936$--$0.996$, and the gap between them
widens monotonically as the trap weakens---at $N{=}12$ from $0.006$ at $\omega=1$ to $0.026$
at $\omega=10^{-3}$. Shell radii alone explain most of the pair structure at strong
confinement; polygonal order explains a growing remainder as the molecule forms.
...
rotation-registered angular order departs from its random-angle null only in the last decade
of confinement (Appendix~\ref{app:structural-tables:shells})---radial stiffening precedes
angular ordering rather than accompanying it. Lab-frame order stays near zero throughout, as
...
fixed spin-projection sector, and we have not scanned total spin, so these are not proven
...
These structural facts also enter the method. The shell radius approaching its classical
$\omega^{-2/3}$ scaling---to within $1.7\%$ of the classical prefactor at $\omega=10^{-3}$,
Appendix~\ref{app:structural-tables:radius}---and the angular order sharpening in the last
decade are the quantities a sampler must respect in order to reach the Wigner regime without
MCMC. Chapter~\ref{ch:kernel} uses them directly: an origin-centred proposal
...
The Jastrow and the backflow are partial substitutes: how the labour is divided between them is a nearly
...
Nothing in the architecture imposes this behaviour: regime changes appear as a rotation of
a low-dimensional axis across branches rather than as growth in latent dimensionality.
...
backflow barely affects the PINN output ($|\delta f|/\sigma_f < 0.07$ everywhere). The
