...
$\varepsilon_{\rm bf}=10^{-6}$ is fixed rather than tuned, and is a numerical guard rather
...
\begin{equation}
  ...
  \label{eq:bf-com}
\end{equation}
taken over particles within each configuration rather than across the batch. Messages are
...
\paragraph{Implementation.} The conventional network is \texttt{BackflowNet}, with
$\phi_\beta$ and $\psi_\beta$ its message and node MLPs; the copresheaf network is
\texttt{CTNNBackflowNet}, whose embeddings, transport maps and update MLPs realise
\eqref{eq:ctnn-edge}--\eqref{eq:ctnn-node}. Aggregation is selectable
(\texttt{sum|mean|max}), spin handling produces the mask $w_{ij}^{\rm spin}$, and the bounded
output of \eqref{eq:bf-bound-scale} is \texttt{out\_bound="tanh"} with
...
configurations resemble electrons spread around the ring rather than clustered at the centre.
