...
backflow, which reshapes the nodes. Each can be built either as a \emph{separable}
network that treats every particle independently, or as a \emph{message-passing}
network in which the particles exchange learned messages before a quantity is read
out.
...
\begin{enumerate}
  ...
  \item \textbf{Message passing as a relational channel, made precise by the
        ...
        rank exactly where a conventional, per-particle backflow collapses to a single
