...
a \emph{Jastrow} correlator $f$, which reshapes the amplitude, and a coordinate
\emph{backflow} $\Delta\mathbf{x}$, which reshapes the nodes. Each can be built in
one of two ways---as a \emph{separable} network that treats every particle on its
own, or as a \emph{message-passing} (CTNN) network in which the particles first
exchange learned messages on a graph and only then read out a quantity. The
...
One boundary of this claim deserves to be drawn explicitly. The VMC equivalence of
...
five on a ring at radius $1.334\,\omega^{-2/3}$~[CITE NEEDED: classical few-electron
Wigner-molecule ground-state configurations]---which in oscillator units sits at
