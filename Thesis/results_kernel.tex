...
\emph{message-passing} (CTNN) network that exchanges and updates node and edge features
over several rounds before a quantity is read out. The
...
coordinates the Hamiltonian actually couples; the DeepSet spreads the same
solution over more directions because it pools independently-encoded pairs rather than
routing them through shared, iterated messages.
...
(ii) \textbf{At Wigner the iterated messages \emph{are} the backflow}: within the CTNN,
...
Q1a and Q1b are one statement about \emph{iterated} relational processing. The
...
pairwise messages---but neither lets that relational information propagate across the
graph. The copresheaf CTNN does: it maintains explicit edge state and transports it
between nodes and edges over several rounds. Where a compact representation exists this
\emph{compresses} the correlation onto fewer tangent directions (Jastrow: smaller
$d_{\mathrm{eff}}$); where the physics demands a genuinely collective, full-rank
response the single-round baseline instead collapses to a single collective displacement mode
(backflow: rank~1), which the iterated transport avoids. Both effects are visible only
...
\paragraph{The weak form gives up the zero-variance property.} For an exact eigenstate
the strong-form local energy $E_L=\hat H\Psi/\Psi$ is constant over configuration space,
so $\Var(E_L)\to0$ as the ansatz approaches the ground state---the zero-variance
principle, and the reason $\Var(E_L)$ is a sharp quality measure. That cancellation is a
property of the \emph{full} local energy, in which the Laplacian (kinetic) and potential
terms combine to a constant; it is not a property of the Laplacian term alone. The weak
(first-derivative) form never forms $E_L$: it replaces the Laplacian-bearing estimator by
a Laplacian-free one, forgoing the exact cancellation and so carrying irreducibly higher
variance. This is why the importance-sampled energy readout $E_{\mathrm{weak}}$ is biased
and must not be used as the convergence criterion---overlap and $\Var(E_L)$ are.
...
single-round-message network collapses to a single collective displacement mode and its energy fails---at
the Wigner crystal, and by capturing Coulomb rather than kinetic correlation. The
