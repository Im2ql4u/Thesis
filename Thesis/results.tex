...
These tables are the thesis's only contact with an external benchmark, and two features of
them carry more information than the energies themselves. The first is \emph{where the
...
These structural facts also enter the method. The shell radius growing as
$\omega^{-2/3}$ and the angular order sharpening as the trap weakens are the quantities a
sampler must respect in order to reach the Wigner regime without MCMC. Chapter~\ref{ch:kernel} uses them directly: an origin-centred proposal
...
between them is weak at strong correlation and strong at moderate coupling, where the
displacements align with the correlator's gradient, and the change is sharp and coincides
with the crossover in $\Gamma$.
