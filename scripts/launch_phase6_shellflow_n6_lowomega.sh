...
RUN_ID="${RUN_ID:-2026-04-12_phase6_shellflow_n6_lowomega}"
...
worker() {
  ...
  for seed in "${SEEDS[@]}"; do
    local tag="shellflow_p6_n6w${omega_tag}_s${seed}"
...
echo "[$(date '+%F %T')] Launching Phase-6 shellflow N=6 low-omega campaign: ${RUN_ID}" | tee -a "${OUT_DIR}/launcher.log"
for i in "${!OMEGAS[@]}"; do
