...
distribution fitted to the largest of them. The relevant thresholds are statements about
moments. For $\hat k<1/2$ the weights have finite variance and the reweighted estimator
obeys a central limit theorem. Between $1/2$ and $1$ the variance is infinite though the
mean still exists, and convergence is slow enough that $\hat k\approx0.7$ is the practical
limit of trustworthiness. Above $1$ the mean itself is undefined, and no sample size
repairs it. An ESS can look survivable while $\hat k$ says the
...
above (shared Slater reference, strictly positive correlator), not by the bound. These quantities---together with the energetic price of ablating a
