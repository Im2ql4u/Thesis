...
are \emph{large}, and with these sample sizes the two come apart. The exact null of
Eq.~\eqref{eq:phi-null} is the right baseline for independent uniform angles, but it is not
quite the null of this pipeline. The angles that reach $|\Phi_n|$ have passed through shell
assignment, sector conditioning and quality cuts, and those condition them. At strong
confinement the $N{=}6$ ring sits $1\%$ \emph{below} the exact null, which says as much: a
departure of one percent in either direction is at the resolution of this comparison, not
evidence of order or of its absence. A null generated by passing angle-randomised frames
through the same pipeline would remove that ambiguity; it is the natural next step and we
have not taken it.

The conclusion that survives is the one that matters: statistical detectability is not what
separates the regimes here, magnitude is. On that criterion the ordering is flat through
$\omega=0.1$, weakly present at $\omega=0.01$ and clearly so only at $N{=}6$, and
unambiguous at $10^{-3}$, where every ring exceeds the null by $16$--$63\%$---far beyond any
plausible difference between the two nulls.
