...
neural tangent kernel $K = OO^{\!\top}$~\cite{JacotEtAl2018-NTK,StokesEtAl2020-QuantumNaturalGradient}. The architecture determines the spectrum of $S$;
...
the finding, which is a collapse to rank $1$---two orders below either ceiling. Toward Wigner the \emph{conventional} backflow collapses to a single
...
\begin{figure}[htbp]\centering
  ...
  \caption[What message passing buys, by ablation]{What message passing buys, by ablation (trained networks, $N{=}6$; energy
...
illustration in this thesis of why energy alone does not certify a state.
$\mathcal{S}^2$ is itself a ratio estimator built from the same degenerate weights, so ``$\to0$'' should be read as \emph{unresolvable at this ESS} rather
...
offers richer nodal topology, and which has been developed into a scalable neural
ansatz by Gao and G\"unnemann~\cite{GaoGuennemann2024-NeuralPfaffian}.
...
$E{=}20.1613\pm0.0023$ ($+0.010\%$). Both lie within $0.5\sigma$ of the reference on the
combined uncertainty, which is dominated by ours: the deviation is some tens of times the
DMC error bar of $8\times10^{-5}$.
...
\begin{table}[htbp]
  \centering
  \caption[Reliability campaign: 30 runs across five confinements]{Reliability campaign: relative energy errors for
...
\begin{table}[h]
  \centering
  \caption[Pareto $\hat{k}$ by regime and recipe]{Average Pareto $\hat{k}$ of the importance weight
