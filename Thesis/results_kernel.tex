...
\chapter[Why the method works]{Why the method works: geometry, optimisation, and sampling}
\label{ch:kernel}
...
Consider what remains unexplained. Two networks of the same size, trained the same way and
landing on the same energy, need not be the same object. A natural-gradient step that is
decisive in one training regime does nothing in another. An importance-sampled objective
reproduces a Markov chain's answer in one corner of the phase diagram and parts company
with it in the next. These look like three unrelated puzzles, and they share one
geometric answer.
...
neural tangent kernel $K = OO^{\!\top}$. Architecture fixes the shape of that geometry,
the optimiser inverts it, and the sampling measure decides whether it can be estimated at
all. The chapter takes them in that order.
...
Three constraints hold throughout. Architectures are compared on a
...
We start with what message passing changes in the tangent geometry, in the amplitude and
in the nodes; then ask what that geometry implies for the optimiser and for the measure
that builds it; and then, treating the measure as something we may choose, remove the
Markov chain from training altogether and find where the choice stops working. A caveat
runs underneath all three and is itself a result: quantities living on a single
component, such as the backflow's rank or its displacement magnitude, are gauge-like,
fixed by the training route rather than by the state. \S\ref{sec:synthesis} sets out what
the conclusions rest on instead, once there are conclusions to rest.

% =====================================================================
\section[Message passing and the variational geometry]{Message passing reorganises the variational geometry}
\label{sec:kernel-q1}
...
% =====================================================================
\section[Conditioning]{Conditioning: what the optimiser can do depends on the measure}
...
($+0.24$--$0.29\%$ across three seeds, Table~\ref{tab:reliability}). The two situations
differ in the sampler and in the quantity being described. The sampler first: the collapse measured above is for the \emph{static} origin-centred
...
(Table~\ref{tab:collocation}). The rest of this section is why it works, where it stops, and what the stopping is made
of.
...
the pipeline achieves and where it fails. Two methodological findings shape what follows. First, among the optimisers (\S\ref{sec:sr-stage}), \textbf{Adam} was the robust
...
The obvious explanation is wrong, or at least incomplete. Ill-conditioning is what makes
natural gradients \emph{useful}; a well-conditioned metric is one Adam can handle
unaided. What disqualifies them is the failure to \emph{estimate} the metric. $F$ is
...
So there are three regimes, not two. Where the metric is well conditioned, SR is sound
and unnecessary. Where it is anisotropic and still estimable, SR is at its most
...
\section[A proposal built from the physics]{A proposal built from the physics, and the wall beyond it}
...
\subsection[The remaining wall]{The remaining wall: a fine-tuned Slater--Jastrow balance, not the sampler}
...
The conclusions of this chapter rest instead on four measurements that survive a change of
gauge. Energy and ablation---delete a component and price it---carry the architecture
comparison. $\Var(E_L)$ separates the two families where energy cannot and where
$d_{\mathrm{eff}}$ has stopped trying, being lower for the message-passing ansatz at every
confinement by factors of two to seven at matched protocol and production capacity
(Table~\ref{tab:varEL}), and seed-cleanly so at the crystal. State overlap makes the
comparison between training paradigms a claim about states rather than about numbers that
happen to agree.

Rank belongs to neither list. It runs through this chapter as the clearest signature of the
mechanism, a field collapsing onto a single mode being easy to see, but what carries the
