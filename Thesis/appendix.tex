...
a measure of what MCMC-free training can achieve. Second, this analysis is what motivated the shift to the weak form, and is reported as a
negative result rather than as a headline number.
...
alone. The floor is nonetheless a property of the objective, because it does not move under
...
% =========================================================
\chapter[Post-catch-22 experimental history]{Post-catch-22 experimental history and philosophy}
\label{app:postcatch22}

\begin{quote}\small\itshape
This appendix is a historical record. It documents the experimental programme as it stood
after the catch-22 analysis and \emph{before} the physics-informed proposal of
\S\ref{sec:kernel-q3-frontier}, and several of its working conclusions were later
superseded---most importantly its reliance on CG-SR, which Chapter~\ref{ch:kernel}
subsequently abandoned below $\omega\simeq0.01$ in favour of Adam. Where it disagrees with
Chapter~\ref{ch:kernel}, Chapter~\ref{ch:kernel} governs. It is retained because the
$N{=}20$ collocation energies of \S\ref{app:postcatch22:frontier} are recorded nowhere else,
and because the shape of the search is itself part of the method.
\end{quote}

This appendix documents the experimental program executed after the
...
branches, and the partial recoveries.

\section{The experimental programme}
\label{app:postcatch22:programme}

After the catch-22 analysis the work shifted from searching for a single better architecture to
engineering a training pipeline that makes a physically strong ansatz trainable across regimes.
Four principles organised it: keep the Slater--Jastrow--backflow ansatz but treat sampling and
preconditioning as first-class objects; replace single long runs with staged continuation
(solve easy regimes, warm-start hard ones); separate claims by evidence tier (online probe,
restored VMC-best, heavy exact VMC); and record negative results, because many theoretically
plausible variants fail on gradient-noise geometry.

\paragraph{What was varied.} We varied the \emph{sampling} (Gaussian and Gaussian-mixture
proposals scaled by $1/\sqrt\omega$; oversample-then-resample with ESS diagnostics; ESS-adaptive
oversampling; weight tempering and log-weight clipping), the \emph{objective} (finite-difference
collocation residuals; a hybrid weak form; and pure REINFORCE, which became the reliable default,
with the Laplacian kept in the reward path but never differentiated), the \emph{preconditioner}
(Adam; diagonal-Fisher natural gradient; and full SR in Woodbury/CG modes with damping anneals
and trust-region caps), and the \emph{training style} (single sweeps; wave-based cascades with
checkpoint inheritance; asynchronous campaigns that relaunch from the best available checkpoint).
Two empirical conclusions were robust. First, optimisation quality dominated architectural
novelty: moderately sized BF+Jastrow models with stable training beat more ambitious variants
with weaker optimisation geometry. Second, at low $\omega$ regularising the resampling
distribution (ESS control, top-mass reduction) helped while hard cusp gating alone did not---a
controlled $2{\times}2{\times}3$ matrix (hard gate on/off, resampling-regularisation on/off,
three seeds) confirmed both, with no measurable gain from gating and a consistent gain from
resampling regularisation.

\paragraph{Outcomes.} At the time of this programme, SR-style preconditioning in CG mode,
warm-started along the confinement cascade
$\omega:1.0\!\to\!0.5\!\to\!0.1\!\to\!0.01\!\to\!0.001$, appeared to be what carried
performance from solved regimes into harder ones; cold starts at low $\omega$ did not
converge. The cascade survived that assessment and is still essential
(Table~\ref{tab:colloc-variants}); the attribution to CG-SR did not. Later runs at matched
budget showed CG-SR diverging below $\omega\simeq0.01$ where Adam succeeded, which is the
three-regime result of \S\ref{sec:kernel-cond}: what fails at low $\omega$ is the
\emph{estimate} of the metric, not the cascade. The full
set of best energies is in \S\ref{sec:colloc-results}; representative cascade successes are
$N{=}6$ at $\omega{=}1.0$ ($E=20.16167\pm0.00181$, $+0.012\%$), $\omega{=}0.5$
($E=11.78466\pm0.00199$, $-0.002\%$) and $\omega{=}0.1$ ($E=3.56525\pm0.00046$, $+0.32\%$), and
$N{=}12$ at $\omega{=}1.0$ ($E=65.70699\pm0.00595$, $+0.010\%$). For $N{=}2$ the collocation route
now covers the whole confinement range cleanly---$+0.012\%$, $+0.023\%$, $-0.004\%$, $+0.002\%$
and $+0.154\%$ at $\omega=1,0.5,0.1,0.01,0.001$ (zero-variance extrapolated)---so the paradigm is
DMC-accurate wherever the importance sampling stays healthy and degrades only in the deep-Wigner
limit.

\section{Current frontier: low-$\omega$ and high-$N$ stabilisation}
\label{app:postcatch22:frontier}

At the close of this phase the unresolved front was the combination of extreme correlation
...
operational stability but did not close the accuracy gap at the hardest targets. What
eventually did, for low $\omega$ at $N{=}6$, was not a stabilisation measure at all but a
change of proposal: the Wigner-ring construction of \S\ref{sec:kernel-q3-frontier}, which
postdates everything in this appendix and carries the cascade to $\omega=0.0035$.
...
result was $+5.53\%$ ($E=31.637\pm0.008$). The gap to
...
\item SR-style preconditioning was judged central for progress beyond early Adam-only
  basins. This is the one conclusion of the six that did not survive: at matched budget
  CG-SR converges faster than Adam at $\omega\ge0.1$ and diverges below it
  (\S\ref{sec:colloc-negatives}), so the operative criterion is whether the metric is
  estimable, not whether it is preconditioned.
...
collects them for the whole grid from a single analysis pass, so that the trends can be
read off rather than assembled from the text. Shell radii are given in trap units
...
Read that way the result is sharper than the raw values suggest. At $\omega\ge0.01$ the
ratio is $1.00$--$1.03$ at every particle number and on every ring---the measured bond order
is indistinguishable from randomly placed angles. Only at $\omega=10^{-3}$ does it depart:
$1.65$ on the $N{=}6$ ring, $1.19$ and $1.56$ on the two rings of the $N{=}12$ $(3,9)$
molecule, and $1.19$ and $1.47$ on the outer two rings of the $N{=}20$ $(1,7,12)$ molecule.
Angular order in these dots is not a gradual strengthening across the confinement range; on
this measure it switches on in the last decade. The Lindemann numbers of the same table fall
monotonically throughout, so radial stiffening precedes angular ordering rather than
accompanying it---which is the separation between radial and angular melting anticipated in
\S\ref{sec:wigner-markers-theory}.

\begin{table}[H]
\centering
\caption{Shell-resolved diagnostics at the modal topology, all frames, for every
$(N,\omega)$. Occupancies are $(n_{\rm in},\dots,n_{\rm out})$; $\tilde r_s$ are mean shell
radii in trap units; $\lambda_s$ are the per-shell angular Lindemann numbers; $|\Phi_{n_s}|$
is the rotation-registered bond order on shell $s$; and the final column is $|\Phi_{n_s}|$
divided by its random-angle expectation, Eq.~\eqref{eq:phi-null}. Values of $1.00$ in that
column mean no measurable angular order. Single-particle inner shells are omitted from the
...
\toprule
$N$ & $\omega$ & occ. & $\tilde r_s$ & $\lambda_s$ & $|\Phi_{n_s}|$ & ratio to null \\
\midrule
...
\begin{table}[H]
\centering
\caption{Shell-model fit to the pair-distance distribution. The empirical
...
\begin{table}[H]
\centering
\caption{Fitted outer-shell radius of the $N{=}6$ $(1,5)$ molecule against the classical
