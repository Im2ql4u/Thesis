...
analysis in Chapter~\ref{ch:kernel}. The architecture size is identical for all $N$. The correlator is the same in both and is
the \emph{separable} network of \S\ref{subsec:separable-correlator} (code: \texttt{PINN});
the message-passing correlator belongs to the architecture study of
\S\ref{sec:kernel-q1} and does not appear in this chapter. It carries
$\approx\!54\text{k}$ parameters, the conventional backflow adds
$\approx\!51\text{k}$ (total $\approx\!105\text{k}$), and the message-passing backflow adds
$\approx\!182\text{k}$ (total $\approx\!236\text{k}$). The two are not capacity-matched, and
at equal width they cannot be: the copresheaf network's persistent edge state is the heavier
object by construction (\S\ref{sec:hyperparameters}). The two production ans\"atze share the
same correlator and differ only in this component, so they are named for it throughout:
\emph{conv.\ BF} and \emph{MP BF}. These are not the widths of
...
\begin{table}[htbp]
  \centering
  \caption[Ground-state energies against the available references]{Residual-based pretraining + SR tail. Reference energies and our conv.-BF
  and MP-BF ground-state energies (Hartree) for selected $(N,\omega)$.
For $N\in\{6,12,20\}$ the references are the fixed-node diffusion Monte Carlo energies of
  ...
  Ref.~\cite{Nordhagen2023-NeuralNetworksFermions}; the $N{=}2$ entries are drawn from the same
  compilation, whose reference column is assembled from H{\o}gberget's DMC and from Taut's
  analytic two-electron solutions~\cite{Taut1994-TwoElectrons}, which exist only at a
  discrete set of frequencies. We do not attribute individual $N{=}2$ rows to one source or
  the other, except at $\omega=1$, where the tabulated $3.00000$ is the known exact result.
  The fixed-node reading of \S\ref{sec:energy-discussion} therefore applies wherever the
  reference is DMC, which is every $N\ge6$ entry.
  ...
  matched} ($\approx105$k against $\approx236$k parameters); the controlled architecture
...
a freely rotating molecule. The cross-shell quantity $\tilde C_{\mathrm{global}}$ depends on
the registration convention and its analysis code was not retained
(\S\ref{subsec:orient}), so it is reported here as indicative; the per-shell moduli and
Lindemann numbers below are rotation-invariant and carry no such dependence.
