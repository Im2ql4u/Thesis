...
function $\nabla_\theta \log|\Psi|$.
The score function is typically smooth even near nodes (where $\Psi$
changes sign but $\log|\Psi|$ merely diverges logarithmically, and the
gradient $\nabla_\theta \log|\Psi|$ remains bounded for the neural
architectures used).
This decoupling---using the Laplacian for \emph{signal} but not for
\emph{gradient flow}---is what allows the collocation pipeline to train
backflow and Jastrow parameters stably in regimes where direct residual
minimisation fails.
