...
This chapter interprets the results of Chapter~\ref{ch:results} through
the three-layer framework of the introduction.
The three decisions---what to parameterize, how to train, and how to
optimize---are not independent in their consequences; understanding where
the method succeeds and where it breaks requires reading all three together.

We begin with the collocation pipeline (Layer~3), because what the optimizer
can and cannot do sets the context for reading the accuracy benchmarks.
The energy benchmarks (Layer~2) then establish where the method sits
relative to the broader hierarchy of quantum chemistry.
The structural diagnostics confirm that the trained states are physically
correct, not merely energetically competitive.
The representation analysis closes the loop to Layer~1: what the network
has actually learned about the ansatz's function space, and what that
implies for architectural choices at larger $N$.

\section{Discussion of the collocation pipeline}
...
in Eq.~\ref{eq:local-energy}), and gradients act exclusively on the score
function $\nabla_\theta \log|\Psi|$.
...
\paragraph{The hidden cost of errors in log-density.}
A concrete illustration of this fragility is an importance-sampling bug
that corrupted training silently for an extended period.
The log-probability used in importance weights was computed as the density
of the individual Gaussian component that generated each sample, not the
mixture density $q(\mathbf{R}) = \tfrac{1}{K}\sum_k \mathcal{N}(\mathbf{R};\,0,\sigma_k^2 I)$.
The correct evaluation---a \texttt{logsumexp} over all components---was
already implemented but never wired in.
In a configuration space of dimension $Nd$, this error introduced a bias
of order $Nd$ nats in $\log q$, corresponding to importance weights
corrupted by a factor of $e^{Nd}$.
At $N{=}6$, $d{=}2$, $\omega{=}0.01$, this meant weights off by a factor
of $e^{12}\approx 160{,}000$.
The failure was invisible in tight traps ($\omega{=}1.0$), where the three
Gaussian component widths overlap sufficiently that the component density is
a reasonable approximation to the mixture density, and catastrophic at low
confinement where the width spread grows as $1/\sqrt\omega$.
This explains why systematic anomalies at low $\omega$ and large $N$ had
been attributed to architecture insufficiency for months: the symptom and
the misdiagnosis are the same function of $(N,\omega)$ as the bug.
The episode is a practical lesson: in high-dimensional importance sampling,
a per-dimension error in log-density is not a small perturbation but an
exponential corruption of the estimator.
...
The force-alignment diagnostic in §\ref{sec:bfforce} makes the regime
switch mechanistically explicit: at $\omega\geq 0.1$ the CTNN displacement
aligns with the confining trap force (cos\,$\approx+0.97$) and opposes
Coulomb repulsion, while at $\omega{=}0.001$ both signs reverse and the
displacement instead follows the Coulomb force---a direct signature of the
backflow switching from a trap-restoring correction to a lattice-correction
mode at the Wigner crossover.
...
correlator features remains close to~1 and rarely exceeds~3
...
Backflow behaves very differently.
For $N{=}6$ and $N{=}12$ the displacement field $\Delta x$ has an
effective rank $r_{\rm eff}(\Delta x)$ substantially larger than the
correlator's---typically $10$--$22$---with relatively flat PCA spectra
and gradual reconstruction under PC ablation.
Linear probes from $\Delta x$ PCs to global observables such as $r_{\rm mean}$,
$r_{\rm var}$, or shell contrast yield negligible $R^2$: the backflow field
does not act as a simple global rescaling or shape parameter.

Near-field diagnostics of $\Delta E$ and $\|\Delta x\|^2$ support a
predominantly local interpretation.
At intermediate traps (e.g.\ $N{=}6$ or $N{=}12$ at $\omega{=}10^{-2}$),
configurations in the lowest $5\%$ by minimum pair distance $r_{\min}$
carry approximately six times their fair share of the backflow energy
correction $\Delta E$, while their share of $\|\Delta x\|^2$ remains
close to unity.
Many spatial modes contribute to the displacement field, but its
\emph{energetic leverage} is concentrated in short-distance configurations
where small nodal adjustments have a large impact on the local energy.
