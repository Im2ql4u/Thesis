...
\end{equation}
for a batch of $B$ configurations drawn from $|\Psi_\theta|^2$ and $P$ parameters. Its
centred Gram matrices define two dual objects. The \emph{quantum geometric tensor}
(QGT), or Fisher metric of the state,
\begin{equation}
  S = \tfrac{1}{B}\,O^{\!\top}O \in \mathbb{R}^{P\times P},
\end{equation}
is exactly the $F(\theta)$ above specialised to $p_\theta=|\Psi_\theta|^2$; stochastic
reconfiguration is natural-gradient descent under this metric, $\dot\theta =
-S^{-1}\nabla_\theta E$~\cite{BeccaSorella2017-QMCBook,StokesEtAl2020-QuantumNaturalGradient}.
Its Gram dual, the \emph{neural tangent kernel} (NTK)~[CITE NEEDED: Jacot et al.,
