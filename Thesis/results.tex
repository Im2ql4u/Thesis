...
solution for $N{=}2$~\cite{Taut1994-TwoElectrons}. Those references reach down to
$\omega=0.1$ for every $N$, and to $\omega=0.01$ for $N{=}2$; below that, for $N\ge6$, no
published value exists, and the boundary is marked wherever it occurs. Section~\ref{sec:wigner-molecule} then uses the trained wavefunction to
...
Below $\omega=0.1$ no reference of any kind exists for $N\ge6$, so the values reported there
are variational predictions rather than validated ones. That boundary is marked wherever it
...
For $N\in\{6,12,20\}$ the references are the fixed-node diffusion Monte Carlo energies of
  H{\o}gberget~\cite{Hogberget2013-DMC}, as tabulated in
  Ref.~\cite{Nordhagen2023-NeuralNetworksFermions}; the $N{=}2$ entries coincide with the
  analytic two-electron results of Taut~\cite{Taut1994-TwoElectrons}. All are benchmarks of
  the same kind, so the fixed-node reading of \S\ref{sec:energy-discussion} applies at every
  size, $N{=}20$ included.
  ...
  \begin{threeparttable}
    \begin{tabular}{c|c|c|cc|cc}
      ...
      6 & 0.001 & ---           & $0.140913(1)$ & --- & $\mathbf{0.140832(1)}$ & --- \\
        ...
        & 1.00 & $20.15932(8)^{\mathrm{H}}$  & $20.1610(6)$  & $+0.0083$  & $\mathbf{20.1585(3)}$ & $-0.0041$ \\
      \midrule
      12 & 0.001 & ---          & $0.515823(3)$ & --- & $\mathbf{0.515365(4)}$ & --- \\
         ...
         & 1.00 & $65.7001(1)^{\mathrm{H}}$  & $65.717(1)$   & $+0.0257$ & $\mathbf{65.69556(5)}$ & $-0.0069$ \\
      \midrule
    ...
    \begin{tablenotes}\footnotesize
      ...
      tabulated in Ref.~\cite{Nordhagen2023-NeuralNetworksFermions}; the $N{=}2$ values are
      the analytic two-electron results of Taut~\cite{Taut1994-TwoElectrons}. The
      foundational DMC study of these dots is Pederiva \emph{et al.}
      \cite{Pederiva2000-QD-DMC} (spanning $N\le13$; see also its
      erratum~\cite{Pederiva2003-QD-DMC-Erratum}), cited here for the method rather than for
      these particular numbers. We took the values from the compilation in the master's
...
\begin{table}[htbp]
  \centering
  \caption[Energies from collocation-assisted training]{Ground-state energies from
  collocation-assisted training, reported under a fixed protocol rather than as the best of
  a hand-tuned search. For $N{=}6$ these are the best single results of the $30$-run
  reliability campaign of \S\ref{sec:colloc-reliability} (Table~\ref{tab:reliability}): a
  fixed $1200$-epoch budget, fixed hyperparameters, two recipes and three seeds per
  confinement. The $N{=}2$ rows are single collocation runs, zero-variance extrapolated.
  The $N{=}12$ rows are marked $^{\dagger}$ because no reliability campaign was run at that
  size: they are the best of the independent runs and are therefore \emph{not}
  protocol-controlled in the sense the other rows are, and should be read as what the method
  reached rather than as what it reaches reliably.
  All values are VMC-evaluated (${\geq}20\text{k}$ samples) at the best checkpoint.
  The reference column is the DMC energy where one exists and, for $(N,\omega)$ with no
  published DMC value ($N{=}6$ at $\omega\le0.01$, and $N{=}2$ at $\omega=0.001$; see
  Table~\ref{tab:energies}), our own converged variational (PINN+CTNN) energy; the
  corresponding \%\,err is then a measure of internal consistency, not of absolute accuracy.
  All rows use the backflow+Jastrow (BF) architecture; the memory-limited $N{=}20$ case is
  discussed separately in Appendix~\ref{app:postcatch22:frontier}.
  Parentheses denote $1\sigma$ statistical uncertainty on the last digit(s).}
  \label{tab:collocation}
  \begin{tabular}{c c c c c c}
    \toprule
    $N$ & $\omega$ & Reference & Energy & \%\,err & Arch \\
    \midrule
    2 & 1.0   & $3.00000$      & $3.00035$      & $+0.012$ & BF \\
      & 0.5   & $1.65977(1)$   & $1.66014$      & $+0.022$ & BF \\
      & 0.1   & $0.44079(1)$   & $0.44077$      & $-0.005$ & BF \\
      & 0.01  & $0.073839(2)$  & $0.073840$     & $+0.001$ & BF \\
      & 0.001 & $0.013778$     & $0.013799$     & $+0.152$ & BF \\
    \midrule
    6 & 1.0   & $20.15932(8)$  & $20.1620(56)$  & $+0.013$ & BF \\
      & 0.5   & $11.78484(6)$  & $11.7850(48)$  & $+0.001$ & BF \\
      & 0.1   & $3.55385(5)$   & $3.5632(7)$    & $+0.263$ & BF \\
      & 0.01  & $0.69036$      & $0.69200(1)$   & $+0.238$ & BF \\
      & 0.001 & $0.140832$     & $0.14118(1)$   & $+0.247$ & BF \\
    \midrule
    12$^{\dagger}$ & 1.0 & $65.7001(1)$   & $65.706(4)$    & $+0.009$ & BF \\
       & 0.5   & $39.1596(1)$   & $39.169(4)$    & $+0.024$ & BF \\
       & 0.1   & $12.26984(8)$  & $12.2824(6)$   & $+0.102$ & BF \\
    \bottomrule
  \end{tabular}
\end{table}

Read against Table~\ref{tab:energies}: at strong confinement the Markov-chain-free route is
statistically consistent with DMC for $N\in\{6,12\}$---the deviations are a few tenths of a
combined standard deviation, though several times the DMC error bar alone, since the
collocation uncertainty dominates--- and it stays within a few tenths of a percent of our own converged
...
decade in $\omega$ is the sharpest transition in the series. The $(1,7,12)$ configuration
itself is not new---it is the known classical shelling and has been reported in quantum
calculations of comparable dots---and no novelty is claimed for it. What we have not found
