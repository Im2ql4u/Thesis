...
def main():
    ...
    ap.add_argument("--no-cusp", action="store_true",
                    help="disable the analytic cusp prior (D2: SR must now learn the stiff cusp mode)")
    ap.add_argument("--omega", type=float, default=1.0,
                    help="trap frequency; low omega = high var(E_L)/kappa, where SR should bite")
    ap.add_argument("--ref-steps", type=int, default=800,
                    help="Adam steps to train the fixed-NTK-basis reference at non-unit omega")
    a = ap.parse_args()
    akw = dict(AKW); akw["use_analytic_cusp"] = not a.no_cusp
    from analysis.system import load_system
    exact = TwoElectronExact(omega=a.omega)
    # Fixed probe set + fixed NTK eigenbasis from a CONVERGED N=2 reference at this omega.
    if abs(a.omega - 1.0) < 1e-9:
        ref = load_system("results/analysis/2026-06-15_N2_w1_ctnn_acc/checkpoint.pt")
    else:
        ref = System(N=2, omega=a.omega, arch="ctnn_vcycle", arch_kwargs=akw, seed=0)
        train_vmc_adam(ref, steps=a.ref_steps, lr=5e-3, batch=2048, log_every=a.ref_steps)
        ref.eval()
    dev = str(ref.device)  # pin every per-seed System to the reference device
    ell = a.omega ** -0.5  # oscillator length: scale the probe set to the wavefunction support
    x_fix = torch.randn(512, 2, 2, device=ref.device, dtype=ref.dtype) * ell
    ...
    for seed in range(a.seeds):
        ...
        s = System(N=2, omega=a.omega, arch="ctnn_vcycle", arch_kwargs=akw, device=dev, seed=seed)
        ...
        train_sr(s, steps=a.steps, batch=1024, lr=0.2, lr_final=0.01, damping=1e-2,
                 ...
                 log_every=a.steps, diag_every=a.every, diag_fn=diag, ref_energy=float(exact.energy))
        print(f"[seed {seed}] done")

    wtag = f"{a.omega:.4f}".rstrip("0").rstrip(".").replace(".", "p")
    tag = ("_nocusp" if a.no_cusp else "") + (f"_w{wtag}" if abs(a.omega - 1.0) > 1e-9 else "")
    out = Path(f"results/analysis/{date.today().isoformat()}_SRmech_N2{tag}")
