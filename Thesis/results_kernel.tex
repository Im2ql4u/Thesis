...
either family's samples or on pooled points. With three and four runs respectively we report
the values rather than a sigma-separation, which at these sample sizes would borrow more
...
the three-seed sets do not overlap and their means differ by $1.8$---not the $7.4$ obtained
by pairing single best runs, which is why the seeded ratio is the one quoted. And capacity
...
widens the ratio to $3.4$ rather than closing it. Intermediate confinements
...
arranged. Quantities defined on a single component---the backflow's rank, its displacement
magnitude, a per-component $d_{\mathrm{eff}}$---are not, because the correlator and the
...
It does not rescue Table~\ref{tab:bf_energies} at all. That gap is a between-network
comparison at unequal size, and nothing in our data separates architecture from the extra
$131$k parameters. It is reported as what the two production ans\"atze achieve---these being
...
that moving one of them moves the other. To test this we held the backflow architecture
fixed---the conventional network in both arms---swapped only the correlator, retrained each
...
Each arm is a separately trained network rather than one network reconfigured, so ``held
...
worth recording because a low effective covariance rank is not what one would predict for a
...
against a classical ring at $3.42\ell$, and a sampler initialised on the shell converges to
the same peak rather than a second basin. No \emph{angular} diagnostic was run at
...
demonstrated angular order. And the chain-free claim is a claim about the gradient---no
