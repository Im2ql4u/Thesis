...
This chapter lays the foundation on which the rest of the thesis rests. We first introduce
the mathematical framework of quantum mechanics, then discuss the challenges of many-body
systems and how second quantization provides a more tractable approach. We then introduce
the model systems used throughout this thesis to benchmark and test our methods, and present
the Hartree--Fock method, a fundamental mean-field approach to approximating many-body
wavefunctions. Finally, we discuss Full Configuration Interaction (FCI), which provides an
exact solution to the many-body problem within a finite basis set, and Variational Monte
Carlo (VMC), a stochastic method for approximating ground-state properties. The following
chapter then shifts to machine learning---statistical learning theory, optimisation, neural
...
In essence, quantum mechanics is formulated in complex vector spaces---specifically
\emph{Hilbert spaces}: complex inner-product spaces that are \emph{complete} in the norm
induced by their inner product, so that every Cauchy sequence converges within the space and
limits and basis expansions are well defined~\cite{Rudin1976-Analysis}. For square-integrable
functions the inner product is
\[
\braket{\phi|\psi} = \int \phi^*(x)\,\psi(x)\, dx,
\]
which, in the physics convention (linear in the second slot), obeys conjugate symmetry
$\braket{\phi|\psi}=\braket{\psi|\phi}^*$, linearity in the ket
$\braket{\phi|\alpha\psi_1+\beta\psi_2}=\alpha\braket{\phi|\psi_1}+\beta\braket{\phi|\psi_2}$,
and positivity $\braket{\psi|\psi}\ge 0$ (with equality iff $\ket{\psi}=0$); it induces the
norm $\|\psi\|=\sqrt{\braket{\psi|\psi}}$. An orthonormal basis $\{\psi_i\}$ satisfies
...
A (pure) quantum state is a normalised vector $\ket{\psi}\in\mathcal H$ (global phase is
...
e.g.\ harmonic-oscillator orbitals for confined charges or plane waves for
...
In a Hilbert space $\mathcal H$, the time evolution of a normalised state $\ket{\Psi(t)}$ is
...
% $\min_{\psi\in \mathcal V_M\setminus\{0\}} E[\psi]$ yields the generalised eigenproblem
% \[
...
% Hermitian positive definite and can be handled by (i) solving the generalised problem directly,
...
% of $\mathbf S$ (thresholding in the Löwdin step) or re-optimises the one-body basis to improve
...
% \paragraph{Normalisation and invariances.}
...
% improve optimiser conditioning.
...
% short-distance behaviour (e.g.\ cusp conditions) tames the kinetic-energy singularities and
% reduces $\mathrm{Var}(E_L)$. In practice we also (i) centre observables, (ii) use large, well-mixed
...
% poorly constrained short-range behaviour, or highly correlated parameters; remedies include larger
...
\]
with the closed-shell density matrix $P_{\lambda\sigma}=2\sum_{i\in\mathrm{occ}} C_{\lambda i} C_{\sigma i}^*$
(sum over occupied \emph{spatial} orbitals; the factor $2$ is the double occupancy that makes the
$\tfrac12$ exchange coefficient correct).
...
initialise $P^{(0)}$, build $\mathbf F[P^{(n)}]$, diagonalize to get $\mathbf C^{(n+1)}$,
...
2D harmonic-oscillator orbitals as the one-particle basis (the Cartesian orbitals used in
our implementation), which respect the shell structure and accelerate convergence. Closed shells admit restricted HF (RHF); spin-polarized
...
($N$, $S_z$, total spin $S$ if spin-adapted CSFs are used, and $L_z$ in an angular-momentum-adapted basis)
...
These methods set the vocabulary rather than supply the numbers. HF fixes what
\emph{correlation energy} means---the part of the exact energy a single determinant cannot
reach---and FCI names the exact answer within a finite one-particle basis, so that
``$0.05\%$ from the reference'' has a scale attached to it. We compute neither here. Our
determinant is built from harmonic-oscillator orbitals rather than HF orbitals, and our
external benchmark is diffusion Monte Carlo~\cite{Pederiva2000-QD-DMC}, which for these
systems is both more accurate than an affordable FCI and available at the particle numbers
we care about. The reader should carry forward the definitions, not expect the tables.
...
The variational principle guarantees that for any normalised trial wavefunction $\Psi_T(\mathbf{R}; \boldsymbol{\alpha})$, the expectation value
\[
E[\Psi_T] = \frac{\langle \Psi_T | \hat{H} | \Psi_T \rangle}{\langle \Psi_T | \Psi_T \rangle}
\]
provides an upper bound to the true ground-state energy $E_0$. Here, $\mathbf{R} = (\mathbf{r}_1,\dots,\mathbf{r}_N)$ and $\boldsymbol{\alpha}$ represents the set of variational parameters. Optimising $\boldsymbol{\alpha}$ minimizes $E[\Psi_T]$.
...
\]
where $T$ is the proposal density (for a symmetric proposal the ratio is unity and the
acceptance reduces to the Metropolis form). When a drift is added, a \emph{quantum force}
\[
...
\subsubsection{Local Energy and Parameter Optimisation}
...
\begin{itemize}
    ...
    \item \textbf{Harmonic Traps:} In systems confined by harmonic potentials, Jastrow factors~\cite{Jastrow1955-ManyBodyCorrelation} like $\exp[-ar_{ij}/(1+\beta r_{ij})]$ model inter-particle repulsion while Slater determinants maintain the proper fermionic symmetry.
\end{itemize}

\subsubsection{Computational Efficiency}
Key factors in achieving efficient QMC simulations include:
\begin{itemize}
    \item \textbf{Efficient Determinant Updates:} For a single-particle move, the Sherman--Morrison
    formula updates the inverse Slater matrix in $\mathcal{O}(N^2)$ rather than recomputing the
    determinant from scratch at $\mathcal{O}(N^3)$.
    \item \textbf{Optimised Gradient and Laplacian Calculations:} Recursive methods and efficient numerical routines for evaluating $\nabla\Psi_T/\Psi_T$ and $\nabla^2\Psi_T/\Psi_T$ are essential.
\end{itemize}

\subsection{Conclusion}
Variational and Diffusion Monte Carlo methods together provide a robust framework for studying quantum systems. VMC offers a flexible approach for optimising trial wavefunctions, while DMC projects toward the ground state through imaginary-time evolution---though
for fermions the practical fixed-node variant is variational only within the imposed
nodal surface, and retains a nodal bias unless those nodes are exact. These methods, working in concert, enable accurate and efficient investigations of systems from simple atoms to strongly correlated materials. Ongoing improvements in trial wavefunction design and stochastic algorithms continue to extend the reach of QMC in modern computational physics.
