...
\begin{table}[htbp]
  ...
  given in the notes below. Parentheses denote the $1\sigma$ blocked statistical error on the
  ...
  and at $N{=}20$ in particular each entry is one training run. ``Rel.\ dev.''
  ...
  $\omega=0.001$, and $\omega=0.01$ for $N\ge6$---have no published value to compare with.
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}\footnotesize
      ...
      above is to their origin. Rows marked \emph{prediction} have no published value.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
...
this pattern. It is not the only thing that would. A difference in the convention for the
Hamiltonian or its units, a time-step or population extrapolation in the reference, an
underestimated statistical error on either side, or an error somewhere in the chain of
compilations by which the reference reached us (see the notes to Table~\ref{tab:energies})
would do the same, and nothing here distinguishes them. The clean test is a fixed-node DMC
...
ratio fall monotonically. The global ratio stays flat because it mixes the ring with the
central electron, whose relative spread grows from $0.43$ to $0.63$ as the ring moves out.
A single-particle width is the weakest of the radial diagnostics for exactly this reason.
...
remainder as the molecule forms. How firmly is a separate matter. Each $\Delta\cos$ comes
from a single split of the frames and carries no interval, and the smallest values---a few
thousandths at $N{=}20$---are of a size that the split, the histogram binning, the shell
threshold or the parametrisation of the angular jitter could move. The growth is consistent
across all three particle numbers, which argues against noise, but it is suggestive rather
than established until it carries a block-bootstrap interval and survives those choices.
...
angular correlations appear to build gradually from strong confinement onward, and
crystalline $n$-fold order emerges from them only in the final decade---three stages, not a
single threshold, with the middle one resting on the weaker of the two measurements.
...
alone. It grows as the trap weakens at every particle number; no uncertainty is attached
to it here (see text). Full table including residuals in
...
``weak'' marks evidence that is suggestive but either small, without an uncertainty
estimate, or carried by a quantity known to mix structures.}
...
$2\langle V_{\rm trap}\rangle/\langle V_{\rm int}\rangle\to1.25,\,1.12,\,1.02$ &
firm at $N{=}12$ \\
...
held-out polygon model against angle-randomised null, $\Delta\cos$ &
weak: small at $N{=}20$, no interval \\
$n$-fold angular order appears late &
$|\Phi_n|$ against its exact null &
firm at $10^{-3}$; within resolution above $0.1$ \\
...
is firm; the gradual middle stage rests on $\Delta\cos$ values that as yet carry no interval.
